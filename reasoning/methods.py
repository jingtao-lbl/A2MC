...
"""
    ...
    # Map target names to output variables (model-aware; FATES's exact legacy
    # heuristic is preserved verbatim when no adapter is active)
    output_names = resolve_output_names(
        results.keys(), targets=targets,
        legacy_fmt=lambda t: f"FATES_{t.upper().replace('_', 'C_')}",
    )
    mechanisms = retrieval_mechanisms()
...
def generate_hypothesis(self, diagnosis: Diagnosis,
    ...
    # Infer mechanisms from likely causes (model-aware; FATES's exact legacy
    # 4-rule keyword scan is preserved verbatim when no adapter is active)
    mechanisms = infer_mechanisms_from_text(" ".join(diagnosis.likely_causes))

    if self.rag_retriever:
        rag_context = self._get_rag_context(
            parameters=param_names[:10],
            mechanisms=mechanisms if mechanisms else retrieval_mechanisms(legacy=('PID_Controller',)),
...
def interpret_results(self, experiment: Experiment,
    ...
    if self.rag_retriever:
        ...
        rag_context = self._get_rag_context(
            ...
            outputs=resolve_output_names(
                output_names[:6], targets=targets,
                legacy_fmt=lambda o: f"FATES_{o.upper()}",
            ),
            query=domain_flavored_query(
                "experiment results interpretation",
                legacy="experiment results interpretation parameter effects",
            ),
        )
...
def extract_lesson(self, experiment: Dict, results: Dict,
    ...
    if self.rag_retriever:
        ...
        rag_context = self._get_rag_context(
            parameters=param_names,
            outputs=resolve_output_names(
                output_names[:6], legacy_fmt=lambda o: f"FATES_{o.upper()}",
            ),
            query=domain_flavored_query(
                f"lesson extraction {outcome}",
                legacy=f"lesson extraction {outcome} experiment mechanistic insight",
            ),
        )
...
"""
    ...
    rag_context = ""
    if self.rag_retriever:
        # Map target names to output variables (model-aware; FATES's exact
        # legacy heuristic is preserved verbatim when no adapter is active)
        output_names = resolve_output_names(
            target_failure_rates.keys(), targets=targets,
            legacy_fmt=lambda t: f"FATES_{t.upper().replace('LEAF', 'LEAFC').replace('FROOT', 'FROOTC')}",
        )
        ...
        rag_context = self._get_rag_context(
            ...
            mechanisms=retrieval_mechanisms(),
            query=domain_flavored_query(
                "screening analysis multi-target calibration parameter sensitivity",
                legacy="screening analysis multi-target calibration parameter sensitivity",
            ),
        )
...
"""
    ...
    if self.rag_retriever:
        ...
        # Map output var to this model's output name (model-aware; FATES's
        # exact legacy map + fallback is preserved verbatim when no adapter
        # is active)
        _fates_output_mapping = {
        ...
        output_name = resolve_output_names(
            [output_var],
            legacy_fmt=lambda v: _fates_output_mapping.get(v, f'FATES_{v.upper()}'),
        )[0]
        ...
        rag_context = self._get_rag_context(
            ...
            mechanisms=retrieval_mechanisms(),
            query=domain_flavored_query(
                f"Morris sensitivity analysis {output_var}",
                legacy=f"Morris sensitivity analysis {output_var} parameter importance",
            ),
        )
