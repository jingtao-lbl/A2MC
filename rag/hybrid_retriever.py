...
def _resolve_collection_from_env() -> Optional[str]:
    """The active profile's ChromaDB collection name, or None to keep the FATES default.

    `_resolve_rag_paths_from_env` already resolves the persist DIRECTORY per profile, but
    nothing resolved the collection NAME inside it -- so every adapter model opened
    FATESVectorStore's default `fates_knowledge`, ChromaDB manufactured an empty collection
    of that name, and the reasoning layer received ZERO vector chunks while the real index
    (`ecosim_knowledge` 1904, `pflotran_knowledge` 1374) sat right beside it. The graph layer
    still answered, so it read as thin recall rather than a miss (20260806b Problem 2).

    Returns None when the profile is unregistered or declares no `collection_name`, which
    preserves the historical behaviour exactly for FATES.
    """
    active = os.environ.get("A2MC_RAG_ACTIVE")
    if not active:
        return None
    rag_dir = os.environ.get("A2MC_RAG_DIR") or str(_A2MC_ROOT / "rag")
    reg = Path(rag_dir) / "milestones.json"
    if not reg.exists():
        return None
    try:
        import json
        milestones = json.loads(reg.read_text()).get("milestones", {})
    except (OSError, ValueError):
        return None
    return (milestones.get(active) or {}).get("collection_name")
...
class HybridRetriever:
    ...
    def __init__(
        ...
        # Resolve the collection NAME per profile, not just the directory. require_existing
        # turns a name mismatch into a loud error instead of an empty auto-created
        # collection that answers every query with silence.
        self.collection_name = collection_name or _resolve_collection_from_env()
        self.vector_retriever = FATESRetriever(
            knowledge_base_path=kb_paths,
            persist_dir=vector_persist_dir,
            auto_build=auto_build,
            collection_name=self.collection_name,
            require_existing=bool(self.collection_name) and not auto_build,
        )
