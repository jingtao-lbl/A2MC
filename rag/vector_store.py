...
class FATESVectorStore:
    ...
    def __init__(
        ...
        #
        # `require_existing` exists because get_or_create is the wrong verb on the READ
        # path. When a reader asks for a collection that is not there, ChromaDB happily
        # MANUFACTURES an empty one and every query then returns nothing -- which is
        # indistinguishable from "the knowledge base has no answer". That silence is how
        # both adapter models retrieved 0 vector chunks for weeks while their indexes sat
        # fully built on disk under a different collection name (20260806b Problem 2).
        # Builders still need create semantics, so the default is unchanged and only the
        # retriever opts in.
        if require_existing:
            ...
            if collection_name not in existing:
                raise ValueError(
                    ...
                    f"{persist_dir}. Present: {sorted(existing) or '(none)'}. "
                    f"Refusing to create an empty one: a manufactured collection returns "
                    f"zero results for every query, which reads as 'no answer' rather than "
                    f"'wrong collection'. Check A2MC_RAG_ACTIVE and the profile's "
                    f"collection_name in rag/milestones.json."
                )
