...
class MemoryManager:
    ...
    def get_relevant_context(self, failing_targets: Optional[List[str]] = None,
        ...
        if relevant_discoveries:
            ...
            for name, disc in sorted_disc[:5]:  # Limit to top 5
                ...
                # `action` is the field that says what to DO about the discovery, and it was
                # dropped here until 2026-08-15 -- so a curated lesson reached the agent as a
                # statement of fact with its instruction silently removed. This was not an edge
                # case: EVERY discovery in EVERY populated store carries `action` (25 of 25
                # across memory/gained_knowledge, memory/ecosim, and the BioCON case) and NONE
                # carries the `implications`/`do_not_repeat` pair that add_discovery() writes
                # and this renderer expected. The stores and the retrieval path had drifted onto
                # different schemas, so the actionable half of the knowledge base was unreachable.
                #
                # Concretely: the BioCON discovery "Score Fs with reduce:
                # growing_season_daytime_mean_abs on the hourly tape. Never compare an all-hours
                # mean..." was curated, was retrieved, and never rendered -- and the all-hours
                # mean was used anyway (6.550 vs the correct 6.979).
                if disc.get("action"):
                    sections.append(f"**ACTION:** {disc['action']}")

                # Provenance, so a claim can be checked rather than taken on trust.
                src = disc.get("source")
                if src and src != "auto_discovered":
    ...
    def add_parameter_info(self, name: str, parameter_pft: Optional[List[int]] = None,
        ...
        """
        # Write-mode gate (2026-08-25). THIS WAS A HOLE: `parameters.json` was written
        # UNCONDITIONALLY while discoveries, failed_approaches and experiments all routed to
        # staging in propose mode, so an unattended online run could write curated parameter
        # knowledge directly — the exact contamination path the gate exists to close (20260612d).
        # `parameters` shapes diagnoses the same way the other three do.
        if self.write_mode == "propose":
            staged = dict(attributes)
            if parameter_pft is not None:
                staged["parameter_pft"] = parameter_pft
            if affects_pfts is not None:
                staged["affects_pfts"] = affects_pfts
            self._stage("parameter", name, staged)
            return
    ...
    def add_parameter_knowledge(self, parameter: str, knowledge_type: str,
        ...
        """
        # Write-mode gate — same hole, same fix (2026-08-25). See add_parameter_info.
        if self.write_mode == "propose":
            self._stage("parameter_knowledge", parameter, {
                "type": knowledge_type, "content": content,
