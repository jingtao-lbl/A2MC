...
export A2MC_SCRIPTS_DIR="~/CaseScripts/Kougarok_FATES"
...
calculate_ensemble_size() {
    ...
    case "$scheme" in
        ...
        lhs)
            echo "$n_samp"
            ;;
        # sobol_seq is the raw scrambled Sobol' SEQUENCE, not the Saltelli design: exactly N rows,
        # like lhs. Added for parity with a2mc_noncime_config.sh's KEEP-IN-SYNC block. Provably
        # inert here -- sobol_seq lives only in the adapter sampler and both FATES rounds are
        # morris -- but it closes the same latent trap, which is the v2.269 precedent for touching
        # this file. Falling through returned 0, worse than an error for a caller sizing headroom.
        sobol_seq)
            echo "$n_samp"
            ;;
        sobol)
            # Second-order flag was ignored here, hardcoding (2P+2) while the samplers honour
            # --no-second-order and emit N(P+2) -- a silent ~2x over-count for a first-order
            # design. Inert on this config today (both FATES rounds are morris) but latent.
