...
A2MC ships as **two public repositories**, split by **how your model is built and run**. Pick by that, not by which sounds newer.

| | [**A2MC-elm**](https://github.com/jingtao-lbl/A2MC-elm) | [**A2MC**](https://github.com/jingtao-lbl/A2MC) (this repo) |
|---|---|---|
...
| adding a model | the built-in FATES path | the `models/` registry + the `onboard-model` workflow |
...
**If your model runs under CIME, use A2MC-elm.** That is where the CIME workflow, the FATES adapter and the Kougarok reference case are developed and shipped, and where that work is strongest.

**If it does not, use this repo.** The adapter layer here exists precisely for models with their own build and run path: describe the model once as a `ModelSpec` plus a backend, and the 7-phase workflow, the knowledge system and the agent skills all apply unchanged.

**This repo also contains the FATES code path**, so it is a superset of A2MC-elm's framework by content. It ships **no case study** — the `use_cases/` surface here is `TEMPLATE/` plus one `*_template` demo case per onboarded model, which is what you copy to start your own. The Kougarok case itself lives in A2MC-elm. The split is about where each line's *development* happens, not about which code is present.

**They converge later.** A2MC is intended to serve both, and **A2MC-elm will fold into A2MC**. Until then the two are developed independently so neither waits on the other.
...
<p align="center">
  <img src="plot/A2MC_Architecture.png" width="100%" alt="A2MC architecture: orchestrator state machine over reasoning, phase scripts, and shared tools">
</p>
...
"Run or test an EcoSIM ensemble/probe"          -> ecosim-run-workflow        (EcoSIM)
...
The **Adaptive Memory** system learns across sessions: generic FATES insights in `memory/gained_knowledge/` and site-specific discoveries under `use_cases/{Model}_{Case}/memory/`, with generalizable lessons promoted from site to generic. At diagnosis/hypothesis time, RAG knowledge, adaptive memory, and current task data are combined into the reasoning prompt (failed approaches marked "DO NOT REPEAT").
...
@software{tao_a2mc_2026,
  ...
  url       = {https://github.com/jingtao-lbl/A2MC-elm},   % the DOI is registered against A2MC-elm
...
**GitHub:** https://github.com/jingtao-lbl/A2MC (non-CIME line) · https://github.com/jingtao-lbl/A2MC-elm (CIME line)
