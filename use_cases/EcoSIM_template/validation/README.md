...
| the sim-vs-obs figure covering every scored target | `ecosim-run-workflow`, with **`plotting`** |
...
Each entry names its variable, its `reduce`, its history tape, its window, the observation and its uncertainty. A target may declare a sub-daily tape and a daytime window.

## Known traps

- **EcoSIM runs a **real Gregorian calendar** -- 366 records in a leap year, no no-leap switch. Never block a tape with `n // 365`; use `tools.calendar_blocks.year_blocks()`.**
...
```bash
python tools/validate_model_targets.py --model ecosim --targets validation/targets.yaml
```
