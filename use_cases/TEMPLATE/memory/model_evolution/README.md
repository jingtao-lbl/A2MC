...
3. **PRESERVE THE BASELINE BEFORE YOU BUILD.** the model builds into per-case for a CIME model; a shared tree for a standalone-binary model. Archive the current binary with its SHA256 and a `PROVENANCE.txt` *first*, **then record it**: `python tools/binary_archive_manifest.py --generate --model generic`.
