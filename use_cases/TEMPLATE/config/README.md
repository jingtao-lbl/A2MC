...
| doing what | skill |
|---|---|
| **running anything at all** — the source order below is step 0 of it | **that model's run-workflow skill** |
...
```bash
source use_cases/TEMPLATE/config/<site>_config.sh          # or the round wrapper, <site>_config_r<N>.sh
```

Since v2.306 **the site config auto-sources the machine config** — `a2mc_config.sh (CIME) or a2mc_noncime_config.sh (adapter)` for this model — when one is not already loaded, and **repairs the wrong one** if it was sourced by mistake. Sourcing the machine config first still works and is a no-op. Order is unchanged: machine defaults, then site overrides, then a round wrapper's overrides.
