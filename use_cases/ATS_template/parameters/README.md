...
| doing what | skill |
|---|---|
| designing a round's list and sampling it | **`phase0-design`**, then `ats-run-workflow` |
...
|---|---|---|
| the XML deck | a nested Teuchos ParameterList, addressed by path | rewritten per case |
...
- **Every value in the shipped template is a `<PLACEHOLDER>` on purpose.** Filling one with a plausible-looking number is worse than leaving it, because a fabricated config looks like a working one ([[feedback_placeholder_targets_structure_not_values]]).
- The grouping axis is **region**, a string axis -- not an integer PFT id. A tool that coerces the axis to an int raises on this list.
