...
3. **PRESERVE THE BASELINE BEFORE YOU BUILD.** ATS builds into a shared CMake tree -- one binary that every build overwrites in place. Archive the current binary with its SHA256 and a `PROVENANCE.txt` *first*, **then record it**: `python tools/binary_archive_manifest.py --generate --model ats`.
