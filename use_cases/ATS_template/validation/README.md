...
| the sim-vs-obs figure covering every scored target | `ats-run-workflow`, with **`plotting`** |
...
Targets come from the deck's **`observations` block**, not from a NetCDF history tape.

## Known traps

- **ATS reports model time from the deck's own timestepper; there is no fixed calendar convention to assume. Read it from the deck.**
...
```bash
python tools/validate_model_targets.py --model ats --targets validation/targets.yaml
```
