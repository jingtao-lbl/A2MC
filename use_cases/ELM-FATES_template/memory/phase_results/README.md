...
| running or submitting | `offline-testing-workflow` |
| a model-source change | **`model-evolution`** (its record goes to `memory/model_evolution/`, not here) |
| a set that crashed mid-run | **`restart-failed-jobs`** |
...
- **Census before scoring.** A CIME case can report success on a leg that produced no usable history. Census the history files before trusting anything. **A crash is not a dead stand** — score neither.
