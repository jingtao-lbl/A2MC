...
3. **The baseline survives a rebuild here.** ELM-FATES builds **per CASE** -- each CIME case owns its `bld/`, so a baseline case is independent and can be built later, so a baseline case is independent and can be built later — the shared-tree hazard that governs the adapter models does not apply. What still applies is binding a run to the executable you mean.
