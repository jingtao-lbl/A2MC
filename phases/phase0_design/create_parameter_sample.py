...
def sample_sobol(
    ...
    """
    # SALib 1.5.2 ships TWO Sobol samplers. Use the CURRENT one:
    #   SALib.sample.sobol.sample(problem, N, *, calc_second_order, scramble, skip_values, seed)
    # The legacy SALib.sample.saltelli.sample has NO seed, emits a DeprecationWarning, and warns
    # about skip_values not being a power of two. This code used the legacy module and passed
    # `skip_values=seed`, which made the seed act as a sequence OFFSET rather than a seed --
    # non-power-of-2 and smaller than N, exactly what SALib complains about (PI catch, 2026-08-18).
    #
    # The modern module gives a real, working seed: same seed reproduces the design exactly,
    # a different seed produces a different one. That is what makes an ensemble re-derivable at
    # analysis time from (problem, N, calc_second_order, seed) alone. `scramble=True` is scipy's
    # randomised Sobol', which improves uniformity and is what makes error estimation meaningful;
    # skip_values is left at the module default (0) because scrambling replaces the need to skip.
