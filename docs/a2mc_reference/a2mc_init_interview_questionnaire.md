...
**A concrete, branching question script for the `onboard-case` skill.** The skill (`.claude/skills/onboard-case/SKILL.md`) is authoritative on the *logic*; this is an illustrative, ready-to-use set of questions the agent can adapt. *(It served `a2mc-init` until 2026-08-02, when the case arc — including this interview — moved to `onboard-case`; the filename is kept so older dev logs still resolve.)* It is **adaptive** in two ways:
