...
- **Weight topic sizing by total source LINE COUNT, not just file count.** File-count parity
  across topics hides density disparities. *Evidence:* EcoSIM's `microbial_bgc` topic (9
  files, 9,811 lines — including the 4,393-line `MicBGCFGMod.F90`, the 3rd-largest file in
  the whole tree) got the same 2-doc budget as much smaller topics like `geochem` (6 files,
  far fewer lines). The resulting ~4,900 lines/doc compression rate (vs. ~2,780 for
  `transport`) meant a calibration-critical mechanism sitting deep in the file — the `RCCZ`
  mortality-recycling equation at line 4132 of 4393 — never made it into the wiki at all,
  despite the same file's earlier content (hydrolysis, catabolism) being well documented.
  Flag any single file over ~2,500–3,000 lines as a mandatory split (`_part1`/`_part2`) or
  dedicated extra subagent pass, independent of how many sibling files share its topic.
  (`memory/dev_logs_adapterkit/20260812a_*`, `20260812b_*`.)
...
| A topic's largest file is disproportionately dense relative to its sibling files | Size doc-budget by total source LINE COUNT per topic, not file count; flag any single file >2,500–3,000 lines for a mandatory split or dedicated pass |
...
**Whatever form the output registry takes (a CDL for NetCDF-native models like FATES/ELM/EcoSIM;
nothing at all for text/deck-based models like PFLOTRAN, whose `output_cdl` is `null` "by nature"
— see `rag/milestones.json`'s own null-reason convention), it is not a substitute for tracing each
calibration-relevant variable's *producing equation* in the wiki prose.** A named output sitting
in a large generic I/O/history file (e.g. EcoSIM's `CO2_SEMIS_FLX_col` in the 5,182-line
`HistDataType.F90`, under `io_and_forcing`) has no natural topic-home for its mechanism — no
subagent's task is ever "trace every named output back to its equation," since that crosscuts
every process topic by construction. Do an explicit cross-cutting pass regardless of format:
**when a structured output registry exists, enumerate targets from it; when it doesn't (no CDL, no
`hist_addfld`-style call to grep), enumerate targets from the use case's actual validation-target
definitions instead** (e.g. `use_cases/<site>/validation/targets.yaml`) — either way, trace each
named calibration-relevant variable to its producing equation, which may span multiple process
topics, and cross-link that trace from both the owning process topic(s) and the output-registry
doc (if one exists). (`memory/dev_logs_adapterkit/20260812a_*`, `20260812b_*`.)
