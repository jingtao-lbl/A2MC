...
| `CanopyStateType` (`biogeophys/CanopyStateType.F90`) | `canopystate_type` container for LAI/SAI (actual and snow-buried), sunlit/shaded profiles, canopy height, displacement, leaf-width, vegetation water potential, plus permafrost active-layer fields including the **new** `altmax_1989_col` and `altmax_ever_col` |
...
| `CanopyTemperature(bounds, ...)` (`biogeophys/CanopyTemperatureMod.F90:45`) | Computes `qg`, `qred`, `t_grnd`, roughness lengths, and the effective `thm` before `CanopyFluxes` runs |
...
`CanopyHydrology` (`biogeophys/CanopyHydrologyMod.F90:100`) runs once per time step, **before** `CanopyFluxes`, to update the canopy water store `h2ocan` and to deliver precipitation (minus interception) to the ground. The body grew by ~162 lines between `60d9aad` and `d40b8431`, primarily from `is_on_soil_col`/`is_on_crop_col` substitutions and reorganized branches around the firn-percolation flag.

### Interception and throughfall

For vegetated soil/crop/wetland patches where `frac_veg_nosno(p) == 1` and precipitation is falling:
...
`dewmx` (maximum allowed dew, mm) is a PFT parameter exposed on `canopystate_type::dewmx_patch` and read from the vegetation-properties file during initialization. The `(LAI+SAI)` dependence implicitly treats rain and snow storage capacity as equal.

### Precipitation onto the ground

`qflx_prec_grnd_rain` and `qflx_prec_grnd_snow` depend on whether the patch is vegetated or not:
...
### Firn-mode capping branches

Several branches inside `CanopyHydrology` (`CanopyHydrologyMod.F90:436, 494, 520, 526, 665`) at `d40b8431` are gated on `use_firn_percolation_and_compaction` rather than `use_extrasnowlayers` (the prior gate). Default behavior with both flags off is unchanged. Activating only `use_extrasnowlayers` no longer triggers these firn-aware branches; that requires `use_firn_percolation_and_compaction = .true.`.

### Irrigation delivery

When `n_irrig_steps_left(p) > 0`, `qflx_irrig = irrig_rate` and the counter decrements. Irrigation water **bypasses the canopy** and is added directly to the ground in the subsequent water-balance code. With `tw_irr = .true.` (two-way coupling to MOSART), the routine distinguishes surface-water irrigation (`qflx_surf_irrig`, arriving on a time-step lag) from groundwater pumping irrigation (`qflx_grnd_irrig`, same time step as demand), and optionally tops up any deficit via `qflx_over_supply`. The trigger time and maximum duration are enforced by `CanopyFluxes` (`irrig_start_time`, `irrig_length`).
...
`fwet` is assigned to `veg_ws%fwet` and `fdry` to `veg_ws%fdry`. Both are zero when the patch has no non-snow vegetation.

`fwet` has three major downstream uses:
1. **Radiation**: `TwoStream` in `SurfaceAlbedoMod` blends leaf optical properties with bulk snow when `t_veg <= tfrz`, using `fwet` as the snow-cover proxy (see [radiation.md](radiation.md)).
...
`CanopyTemperature` (`biogeophys/CanopyTemperatureMod.F90:45`) is the pre-flux step historically known as `Biogeophysics1`. The actual Newton-Raphson loop lives in `CanopyFluxes`; `CanopyTemperature` performs the **pre-computation** that must happen before `CanopyFluxes` and `BareGroundFluxes` can start:
...
After `CanopyTemperature` returns, `CanopyFluxes` (and, in parallel, `BareGroundFluxes`, `UrbanFluxes`, `LakeFluxes`) can run using the stored `qg`, `t_grnd`, and roughness lengths. The branches at `CanopyTemperatureMod.F90:251, 304` use `col_pp%is_soil(c) .or. col_pp%is_crop(c)` (instead of the old landunit-type comparison).
...
with `roota_par(pft)` and `rootb_par(pft)` read from the parameter file, and `zi(c,lev)` being the column's layer-interface depth. For FATES vegetation, `init_vegrootfr` skips the patch (gated on `.not. veg_pp%is_fates(p)`), and FATES populates its own root profile during initialization.

When `use_var_soil_thick = .true.` and the bedrock level `nlev2bed` is shallower than `nlevsoi`, `init_vegrootfr` **renormalizes** `rootfr` so the total over the non-bedrock layers sums to 1, preventing roots from being placed into the impermeable bedrock layers.

Currently only `zeng_2001_root` is implemented; placeholder branches for `jackson_1996_root` and `schenk_jackson_2002_root` exist but `endrun` when selected. `RootBiophysMod` is unchanged at `d40b8431`.
...
if (td >= 0) then                  ! Liquid-water saturation (above 0 C)
   es   = a0 + td*(a1 + td*(a2 + ... + td*a8)) ...
   esdT = b0 + td*(b1 + td*(b2 + ... + td*b8)) ...
else                               ! Ice saturation (below 0 C)
   es   = c0 + td*(c1 + td*(c2 + ... + td*c8))
   esdT = d0 + td*(d1 + td*(d2 + ... + td*d8))
end if
```

Both branches are clamped to `-75 <= T_limit <= 100` C. Units are Pa for `es` and Pa/K for `esdT`. The specific humidity output is the standard form using the ratio of molecular weights (0.622):
...
- `CanopyFluxes` — once before the iteration and again after each Newton step to rebuild `qsatl(p)` at the new leaf temperature
- `BareGroundFluxes`, `UrbanFluxes`, `LakeFluxes`, `SoilFluxes` — each for its own surface

`QSat` is intentionally kept as a pure, `$acc routine seq` subroutine so it can run on the device. `QSatMod` is unchanged at `d40b8431`.
...
`DaylengthMod` provides the geometric daylength used by the CN/CNP phenology. The central function is:
...
with `secs_per_radian = 13750.9871` — the number of seconds per radian of hour angle (86400 / (2π)). Latitudes within `epsilon` of ±π/2 are offset slightly to avoid `cos(lat) = 0`.

`UpdateDaylength(bounds, declin)` is called daily from the main time-step loop. It iterates over all gridcells and stores current and previous day's daylength on `grc_pp`, which are then used by `CNPhenology` to compute day-length accumulation and trigger leaf-on for temperate and boreal deciduous types. `DaylengthMod` is unchanged at `d40b8431`.

## `canopystate_type` container

Declared at `biogeophys/CanopyStateType.F90:36`, the `canopystate_type` holds everything that is "the canopy" — structural state that persists across time steps and is read by radiation, fluxes, and BGC:

- **Vegetation area**: `tlai_patch`, `tsai_patch` (raw, no snow burial), `elai_patch`, `esai_patch` (after burial), `tlai_hist_patch`, `tsai_hist_patch` (satellite-phenology mode), `elai240_patch` (10-day mean).
...
- **Permafrost active layer** (`CanopyStateType.F90:62-71`):
  - `alt_col`, `alt_indx_col` — instantaneous active-layer thickness and its layer index
  - `altmax_col`, `altmax_indx_col` — annual maximum
  - `altmax_lastyear_col`, `altmax_lastyear_indx_col` — prior year's maximum (used as CN rooting bound)
  - `altmax_1989_col`, `altmax_1989_indx_col` — **new** at `d40b8431`: 1989-baseline maximum thaw depth (RF: temporary for IM1; the year is hard-coded as the polygonal-tundra baseline). Allocated at line 150, restart-wired at line 625.
  - `altmax_ever_col`, `altmax_ever_indx_col` — **new** at `d40b8431`: maximum thaw depth from beginning of simulation. Allocated at line 151. Updated in `ActiveLayerMod::alt_calc` whenever `alt > altmax_ever`; reset to zero in spinup mode (`spinup_state /= 0`). Restart-wired at line 628.

The four new fields are written by `ActiveLayerMod::alt_calc` (see [soil_temperature.md](soil_temperature.md)) and consumed only when `use_polygonal_tundra = .true.` (the polygon-specific subsidence and microtopographic-relief updates).

The `canopystate_type` also adds three accumulator procedures at `CanopyStateType.F90:85-87`: `InitAccBuffer`, `InitAccVars`, `UpdateAccVars`.

Two module-level logicals, `perchroot` and `perchroot_alt` (`biogeophys/CanopyStateType.F90:26-30`), switch how `btran` treats frozen soil:
...
Both are set from namelist in `SoilMoistStressMod::set_perchroot_opt` and consumed in `calc_root_moist_stress` (see [canopy_fluxes.md](canopy_fluxes.md)).
...
- `h2ocan` caps the interception evaporation flux in `CanopyFluxes`.
...
- `dayl_patch` from `DaylengthMod` is used by CN/CNP phenology in `biogeochem/PhenologyMod.F90` (and the BeTR-coupled variant `biogeochem/CNPhenologyBeTRMod.F90`) and A2MC's FATES phenology calibration workflow (see `use_cases/ELM-FATES_Kougarok/memory/gained_knowledge/discoveries.json`).
- `QSat` is the lowest-level utility shared across every surface-flux driver.
- `altmax_1989_col`, `altmax_ever_col`, plus the `excess_ice`, `iwp_microrel`, `iwp_subsidence`, `frac_melted` fields on `col_ws`, are all the polygonal-tundra state written from [soil_temperature.md](soil_temperature.md) (`ActiveLayerMod::alt_calc`).
