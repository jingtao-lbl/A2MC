...
- `dayl_patch` from `DaylengthMod` is used by CN/CNP phenology in `biogeochem/CNPhenologyMod.F90` and A2MC's FATES phenology calibration workflow (see `use_cases/ELM-FATES_Kougarok/memory/gained_knowledge/discoveries.json` for the corresponding mechanistic insights).
