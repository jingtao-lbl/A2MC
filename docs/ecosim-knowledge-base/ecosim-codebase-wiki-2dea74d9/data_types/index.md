...
- **No module in this directory defines a Fortran derived type** — the arrays are always plain intrinsic-type (`real(r8)`, `integer`, `character`) module-level allocatables. The only variation is element type: e.g. `EcoSIMHistMod` is notable for holding history/parameter-file name tables as `CHARACTER(len=16)` arrays (`EcoSIMHistMod.F90:11-20`) rather than the usual `real(r8)`, and `PlantTraitTableMod` carries integer PFT trait tables indexed by a single `_tab` dimension (`PlantTraitTableMod.F90:1-60`).
...
A physics module (see for example `f90src/Plant_bgc/GrosubsMod.F90` or `f90src/Microbial_bgc/Box_Micmodel/MicBGCFGMod.F90`) will:
