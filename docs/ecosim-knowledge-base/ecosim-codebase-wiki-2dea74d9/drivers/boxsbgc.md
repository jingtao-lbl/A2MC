...
1. `err_status%reset()` and zero `ystatesfl`.
2. `call SoilBGCOneLayer(I, J, micfor, micstt, micflx, naqfdiag, nmicdiag)`
   — per-step run of the microbial BGC library (microbial decomposition,
   growth, mortality, nitrification, denitrification).
3. Chemistry equilibrium via `ChemMod::RunModel_nosalt(forc, micfor,
   nvars, ystates0l, ystatesfl, err_status)` (see below).
   Steps 2–3 are **both gated by `if(.not.forc%disvolonly)`** — when
   `disvolonly` is set, the microbial BGC and chemistry are skipped and
   only the surface-flux step below runs.
4. `call CalcSurflux(forc, micfor, nvars, ystates0l, ystatesfl, err_status)`
   (defined at line 1281) — dissolution / volatilization fluxes at the
   soil-air interface plus gas exchange.
5. `call UpdateStateVars(...)` — repack into `ystatesfl`.
6. `call UpdateSOMORGM(micfor, micstt)` — roll microbial
