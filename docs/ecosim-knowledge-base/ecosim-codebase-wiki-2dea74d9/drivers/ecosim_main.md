...
1. **Annual initialization (`EcoSIMAPI.F90:370-417`).**
   ...
   - On the first year only (`ymdhs(1:4) == frectyp%ymdhs0(1:4)`):
     - `STARTS(NHW,NHE,NVN,NVS)` — soil state initialization
       (`EcoSIMAPI.F90:383`).
   - Every year, gated only by `plant_model`:
     - If plants active, `ReadPlantInfo(yearcur, yearclm, ...)`
       (`EcoSIMAPI.F90:390`). This block sits outside the first-year
       guard; per the inline comment, plant information is read in every
       year (the active flags come from the checkpoint file).
   - On the first year only, if plants active:
     - `STARTQ(NHW,NHE,NVN,NVS, 1, JP)` — plant state
