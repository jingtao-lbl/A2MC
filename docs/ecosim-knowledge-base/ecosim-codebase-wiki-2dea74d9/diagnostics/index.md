...
The unit-110/unit-111 files are plain ASCII. No explicit `OPEN` exists for either unit (the only `open(111,...)` in the tree is commented out at `ecosim.F90:51`); they are implicitly default-connected (`fort.110` / `fort.111`) on first write.
...
<physics + BGC>
REDIST(I, J, ...)                                ← drivers/ecosim/EcoSIMAPI.F90:113
DiagSoilGasPressure(I, J, ...)                   ← drivers/ecosim/EcoSIMAPI.F90:117 (end-of-step, after REDIST)
EndCheckBalances(I, J, ...)                      ← drivers/ecosim/EcoSIMAPI.F90:119 → ModelDiags/BalancesMod.F90:170
...
Units 110 and 111 have no explicit `OPEN`; they are implicitly default-connected (`fort.110` / `fort.111`) on first write, and contain only failure-time audits rather than a continuous stream.
