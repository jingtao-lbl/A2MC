...
**Last verified:** 2026-08-12 (`CO2_SEMIS_FLX_col` section added from a source-verified investigation
on `adapter-kit`; all other sections verified 2026-04-24)
---
...
- `trcs_Soil2plant_uptake_col(idg,NY,NX)` -- plant uptake (delivered from `Plant_bgc`). `RGasNetProd_col(idg)` is already net of plant uptake when the balance (`errmass`) is computed at `TranspNoSaltMod.F90:188`; line 191 then adds uptake back via `RGasNetProd_col(idg) += trcs_Soil2plant_uptake_col(idg)` (`TranspNoSaltMod.F90:191`) for diagnostic reporting only, not as a subtraction in the balance.
...
### `CO2_SEMIS_FLX_col` — the CO2-specific history output

The CO2 tracer's history field is `f90src/IOutils/HistDataType.F90:4243`:
```
CO2_SEMIS_FLX_col = SurfGasEmiss_all_flx_col(idg_CO2) / AREA_3D * GramPerHr2umolPerSec(idg_CO2)
```
a unit conversion (g C h^-1 per unit ground area -> umol CO2 m^-2 s^-1) of the generic
`SurfGasEmiss_all_flx_col` defined above, with the disturbance-loss adjustment block
(`RedistMod.F90:186-200`) gated on `NU_col>NUI_col` and containing an unconditional `cycle` — always
skipped in an undisturbed run, so `CO2_SEMIS_FLX_col` reduces to a straight unit conversion of
`SurfGasEmiss_flx_col` in practice. This is the field typically used as the soil-CO2-efflux (`Fs`)
calibration target; its **production side** (what determines how much CO2 there is to move) is
`Microbial_bgc` heterotrophic catabolism plus mortality-recycling losses, and `Plant_bgc` root/
rhizosphere respiration — see [`microbial_bgc/decomposition_and_som.md`](../microbial_bgc/decomposition_and_som.md#from-catabolism-to-net-co2-flux-the-pathway-to-co2_semis_flx_col)
for the full production-side chain. Every quantity on this page (the transport side) is either a
hardcoded constant or a site/soil physical property — none of it is a PFT/microbial calibration
parameter, so calibration leverage on `CO2_SEMIS_FLX_col` lives entirely on the production side.
