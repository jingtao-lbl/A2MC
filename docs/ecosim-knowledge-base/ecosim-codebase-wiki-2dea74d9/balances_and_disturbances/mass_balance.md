...
`ApplyTillageMixing` (`TillageMixMod.F90:36-360`) runs only at solar noon when a tillage event is active: its gate (`TillageMixMod.F90:63`) requires `J == INT(SolarNoonHour_col)` AND `XTillCorp_col < 1.0` AND `DepzCorp_col > 0` (otherwise it early-returns). For background, disturbance types are numbered in `SoilDisturbMod.F90:43`, which defines 1-20 as tillage, 21 = litter removal, 22 = fire, 23-24 = drainage. Implementation:
