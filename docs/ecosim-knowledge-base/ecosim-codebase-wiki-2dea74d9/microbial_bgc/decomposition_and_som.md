...
**Last verified:** 2026-08-12 (mortality-recycling, CO2-aggregation, and numerical-hazards sections
added from a source-verified investigation on `adapter-kit`; all other sections verified 2026-04-24)
---
...
**Growth efficiency energetics**: growth respiration efficiencies (fraction of C uptake lost as CO2) are derived from empirical free-energy yields (`NitroPars.F90:214-220`):
...
## From catabolism to net CO2 flux (the pathway to `CO2_SEMIS_FLX_col`)

This section covers two named mechanisms: **heterotroph aerobic catabolism** (the CO2-producing
uptake step above, aggregated into a net flux here) and **microbial maintenance respiration** (the
baseline biomass-proportional CO2 cost, step 2 below).

The catabolic uptake `RGOCP` above is not itself the CO2 flux — three more steps aggregate it into
the quantity that eventually leaves the soil column (traced end-to-end, source-cited, in
`memory/logs/20260811a_Fs_Full_Equation_Trace_Production_Transport_And_Key_Parameters.md` on the
`adapter-kit` branch; summarized here):

1. **O2-limited realization** (`MicBGCFGMod.F90:2806-2811`): `RCO2ProdHeter(K) = RespGrossHeter(K) =
   RGOMP * OxyLimterHeter(K)`, where `RGOMP` is the stoichiometric CO2 yield from the catabolized C
   and `OxyLimterHeter` is the fraction of unconstrained respiration actually realized given O2
   availability (see Oxygen limitation, above).
2. **Maintenance/growth split** (`GatherHetertrophRespiration`, `MicBGCFGMod.F90:4040-4129`):
   ```
   RMaintRespHeter = mBiomeHeter * RMOM * TempMaintRHeter * F_pH * RMOMK
   RGrowthRespHeter = max(0, RespGrossHeter - RMaintRespHeter)
   ```
   with `RMOM = 0.010 g C g^-1 N h^-1` (`NitroPars.F90:209`) the maintenance-respiration rate
   constant. This is a **diagnostic decomposition of the same physical flux**, not two separate CO2
   streams. Two further terms are added onto `RCO2ProdHeter` beyond the base catabolic flux
   (`HeterotrophAnabolicUpdate`, `MicBGCFGMod.F90:1905-1919`): the respiration cost of biological N2
   fixation, and senescence respiration when maintenance demand exceeds supply (the killed biomass
   covering the deficit is also respired to CO2 — see Microbial mortality, below).
3. **Aggregation** (`AggregateTransfOMBioResdue`, `MicBGCFGMod.F90:2122-2262`), summed over every
   guild and kinetic component: `tRCO2MicrbProd = sum(RCO2ProdHeter) + sum(RCO2ProdAutor)`, and
   `RCO2NetUptkMicb = tRCO2GrothAutor - tRCO2MicrbProd - tRNOxMicrbRedux` (`tRCO2GrothAutor` is CO2
   *uptake* by chemolithoautotrophs, e.g. ammonia oxidizers).

This net microbial CO2 term, together with root/rhizosphere autotrophic respiration
(`f90src/Plant_bgc/RootMod.F90`, routed to the same soil-gas source array via
`RootGasMod.F90`), is written into the shared gas-source pool
`RBGCSrceGasMM_vr(idg_CO2,...)` consumed by transport (`f90src/Transport/Nonsalt/InitNoSaltTransportMod.F90:399-408`;
see [`transport/gas_transport.md`](../transport/gas_transport.md) for how this becomes the
`CO2_SEMIS_FLX_col` history output). **The production side sets how much CO2 there is; the transport
side (diffusion, ebullition, wet deposition) sets how fast it leaves** — the two are architecturally
separate, and every transport-side quantity is either a hardcoded constant or a site/soil physical
property, not a PFT/microbial calibration parameter. Calibration leverage on this output lives
entirely on the production side.
...
### Mortality routing: recycled back into biomass, or lost to litterfall/humus

This is EcoSIM's **microbial mortality recycling** mechanism. `GatherHetertrophAnabolicFlux`
(`MicBGCFGMod.F90:4132-4340`) computes the mortality flux and splits
it between two fates. The mortality rate constant compounds the basal turnover rate with an
environment scalar and a low-biomass-concentration penalty (`:4313,4315`):
```
SPOMX = sqrt(GrowthEnvScalHeter(NGL,K)) * SPOMC(M) * SPOMK(M)
RKillOMHeter(NE,M,NGL,K) = max(0, mBiomeHeter(NE,MID,K) * SPOMX)
```
This is an **explicit, unclamped** pool-times-rate update — nothing caps `RKillOMHeter` against the
standing biomass pool the way the hydrolysis and sorption laws elsewhere in this page use `min(0.5,
...)` or an explicit `dHyd <= 1` fraction. At large `SPOMC` this can attempt to remove more carbon
in one timestep than the pool holds (see Known numerical hazards, below).

The killed flux then splits into recycled-back-into-biomass vs. routed-to-litterfall via the
recycling fraction `RCCC` (`:4297-4306`):
```
IF (mBiomeHeter(C,MID3,K) > ZEROS AND mBiomeHeter(C,MID1,K) > ZEROS) THEN
  CCC = min(1, N_ratio, P_ratio)              ! stoichiometric (N/P) sufficiency, 0-1
  CXC = mBiomeHeter(C,MID3,K) / mBiomeHeter(C,MID1,K)   ! reserve:kinetic biomass ratio
  C3C = 1 / (1 + CXC/CKC)                     ! reserve-availability scalar, 0-1
  RCCC = RCCZ + max(CCC, C3C) * RCCY
ELSE
  RCCC = RCCZ
ENDIF
RkillRecycOMHeter(NE,M,NGL,K) = RKillOMHeter(NE,M,NGL,K) * RCCC       ! back into biomass
RkillLitfalOMHeter(NE,M,NGL,K) = RKillOMHeter(NE,M,NGL,K) - RkillRecycOMHeter(...)   ! to litterfall/humus
```
`RCCZ = 0.167` (minimum recycling fraction), `RCCY = 0.833` (the additional fraction unlocked by
`max(CCC,C3C)`), `CKC = 0.001` (`NitroPars.F90:169-170,209`, `:38`; comment at `:4248`: "RCCZ,RCCY =
min, max C recycling fractions"). Only the recycled share stays out of the CO2-releasing pathway;
`RkillLitfalOMHeter` feeds `RkillLitrfal2HumOMHeter`/`RkillLitrfal2ResduOMHeter` (humification split
by `EHUM`), which re-enters the SOM hydrolysis chain above and is eventually respired.

**Runtime-verified example** (BioCON site, PFT1, `CKC` at 0.625x default = 0.000625): a debug print
at `:4306` (temporary, reverted; see `use_cases/EcoSIM_BioCON/memory/logs/20260812b_*.md` on
`adapter-kit`) recorded `CCC` in [0.489, 0.500] and `C3C` in [0.0012, 0.0062] across 30 samples — `CCC`
80-400x larger than `C3C` every time, so `max(CCC,C3C)` picked `CCC`, never `C3C`. **At this site's
default `CKC` scale, `C3C` is structurally negligible relative to `CCC`** — `CKC`'s value has
essentially no effect on the realized recycling fraction unless `CXC` (the biomass ratio) is itself
comparably tiny. Do not assume `CKC` is an effective independent lever on `RCCC` without checking
which term of `max(CCC,C3C)` actually binds at the target site.

**Calibration implication**: `RCCZ` (raising it keeps more mortality carbon *out* of the
CO2-releasing pathway) is the single largest elementary effect on `CO2_SEMIS_FLX_col` found in a
23-parameter Morris screen at this site (`use_cases/EcoSIM_BioCON/memory/logs/20260811h_*.md`,
`20260812a_*.md`) — larger than any single catabolism-rate parameter above.
...
This is EcoSIM's **DOM sorption desorption** mechanism. `RDOMSorption` (`MicBGCFGMod.F90:1218-1305`) models linear sorption-desorption exchange between `DOM(idom, K)` and `SorbedOM(idom, K)`:
...
**Numerical hazard**: like `RKillOMHeter` above, `RDOMSorption` is an **explicit, unclamped**
pool-times-rate update (`DOM -= RDOMSorp; SorbedOM += RDOMSorp`, no flux limiter) — see Known
numerical hazards, below.
...
1. `SPOSC(jsken, jcplx)` (`MicBGCPars.F90:319-325`) for solid SOM specific hydrolysis rates. **At the
   BioCON site this has been confirmed to have ZERO measurable effect on `CO2_SEMIS_FLX_col` across a
   32x tested range, in four independent regimes, confirmed by a runtime debug print** — the term it
   multiplies (`RHydrolysisScalCmpK`) is itself ~1e-6 at this site, set entirely by `DFNS`/`OQCI`/
   `BulkSOMC` (none of which involve `SPOSC`). See `use_cases/EcoSIM_BioCON/reports/20260811a_SPOSC_Investigation_Summary/`
   on `adapter-kit` for the full investigation. Do not assume this null result transfers to a
   different site's SOC/moisture regime without re-checking `RHydrolysisScalCmpK`'s magnitude there.
2. `SPORC(1:2)` and `SPOMC(1:2)` (`NitroPars.F90:210-211`) for necromass and live-biomass turnover.
   `SPOMC` is the single positive-sign parameter with the largest measured elementary effect on `Fs`
   after `RCCZ`/`VMXO` at the BioCON site; `SPORC`'s pathway is double-clamped and numerically safe,
   `SPOMC`'s is not (see Known numerical hazards).
...
9. `RCCZ`, `RCCY`, `CKC` (`NitroPars.F90:38,169-170,209`) for the mortality-recycling split (see
   Mortality routing, above) — `RCCZ` is the single largest measured elementary effect on `Fs` of
   any parameter tested at the BioCON site (negative sign: more recycling, less `Fs`).
10. `TSORP`, `HSORP` (`NitroPars.F90:110-111,205-206`) for DOM sorption/desorption — see the
    numerical-hazard note under DOM sorption, above, before widening this beyond a narrow range.
...
## Known numerical hazards for calibration

Two rate laws in this module are **explicit and unclamped** — no `min(pool, ...)` or `min(1, ...)`
cap prevents them from removing more of a pool in one timestep than the pool holds, unlike every
hydrolysis-family law above (which are all capped at `min(0.5, ...)` or an explicit `dHyd <= 1`
fraction):

- **`RDOMSorption`** (`TSORP`-governed, `:1218-1305`): `DOM -= RDOMSorp; SorbedOM += RDOMSorp`, no limiter.
- **`RKillOMHeter`** (`SPOMC`-governed, `:4313-4315`): `mBiomeHeter -= RKillOMHeter`, only floored at
  zero (`AZMAX1`), not capped against the standing pool.

**Empirically confirmed at the BioCON site** (a 23-parameter Morris sensitivity screen,
`use_cases/EcoSIM_BioCON/memory/logs/20260811g,h_*.md` on `adapter-kit`): raising `TSORP` alone above
~1.5x its default produces a sharp stability cliff — 87-91% simulation success at `TSORP` <=1.5x
default, 0% success at >=2.75x, the signature of a numerical rather than physical failure. Tracing
the residual failures after tightening `TSORP` (`20260811h`'s 2026-08-12 update) found every
remaining crash/blow-up coincides with **`RCCZ` near its tested ceiling while `SPOMC` and/or `SPORC`
are simultaneously elevated** — plausible because `RCCZ` and `SPOMC` compound through the *same*
timestep's recycling equation (Mortality routing, above): fast mortality (`SPOMC` high) combined with
fast recycling of that mortality flux back into the same pool (`RCCZ` high) is a decay-then-regrow-
in-place feedback, the textbook shape for forward-Euler stiffness once both rates are large together.
`SPORC`'s own pathway is double-clamped (`min(1, ...)` then `min(flux, pool)`, see SOM hydrolysis
rate law above) and is very likely an innocent bystander correlated by Morris trajectory geometry,
not a third cause.

**Practical guidance**: when designing a calibration ensemble that samples `TSORP`, `SPOMC`, or
`RCCZ` over a wide multiplier range, expect a nonlinear failure boundary in their joint space, not
independent per-parameter bounds. A `TSORP` range of `[0.25, 1.5]x` default was empirically safe at
this site; `RCCZ`'s safe ceiling has not been precisely mapped (only that it is implicated jointly
with `SPOMC`/`SPORC` near the top of a `[0.25, 4.0]x` range).
