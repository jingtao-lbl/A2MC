...
`HYCDMicP4RootUptake_vr` is the conductivity actually consumed by the plant root water uptake routine (`Plant_bgc/NutUptakeMod.F90`). It is computed in `GetSoilHydraulicVars` (`SoilHydroParaMod.F90:118-119`) as the arithmetic mean of the horizontal (direction 1) and vertical (direction 3) lookup entries at the current moisture bin:
