...
- `CleanUpTracerIDs` (line 135, `use TracerIDMod`) deallocates only `trcs_names`.
...
- `InitPlantMorphSize()` (f90src/Ecosim_mods/InitAllocMod.F90:152-187) moves plant morphology counts in two directions. It copies `JZ`, `NumCanopyLayers`, `JP`, the azimuth/zenith sector counts (`NumOfLeafAzimuthSectors`, `NumOfSkyAzimuthSects`, `NumLeafZenithSectors`) and `MaxNodesPerBranch` **from** `GridConsts` module-level constants **into** `pltpar` (:157-163). Six other counts — `MaxNumBranches`, `MaxNumRootAxes`, `NumGrowthStages`, `NumOfPlantMorphUnits`, `NMaxRootSegs`, `NumLitterGroups` — are **hard-coded into** `pltpar` (10/10/10/7/10/5 at :175-180) and then written **out** to the `GridConsts` module-level variables (:181-186), i.e. `pltpar → GridConsts`. It also copies kinetic-component indices (`iprotein`, `icarbhyro`, `icellulos`, `ilignin`, `k_woody_comp`, `k_fine_comp`) from `micpar` into `pltpar` (:164-169). This mirroring lets plant code use `pltpar` as a one-stop shop without reaching into `micpar`.
