...
**Physics wrapped:** `SoilBGCOneLayer` (`f90src/Microbial_bgc/Box_Micmodel/MicBGCFGMod.F90:67`, in module `MicBGCMod`; reached via `use MicBGCMod`).
...
**Physics wrapped:** `GeoChemEquilibria` (`f90src/Geochem/Layers_chem/SoluteMod.F90:43`, reached via `use SoluteMod`; called from within `soluteModel` at `GeochemAPI.F90:136`, along with `UpdateFertilizerBand` and `UpdateSurfResidueSolute`).
