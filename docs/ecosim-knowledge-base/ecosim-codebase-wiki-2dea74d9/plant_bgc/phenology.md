...
up to `BranchNumMax` branches, where `BranchNumMax(0:3)=(/5,1,1,1/)` is indexed by
`iPlantTurnoverPattern_pft` (all-aboveground=5, else 1; declared at
`PlantPhenolMod.F90:30`) — the branch cap is set by turnover pattern, not by growth form.
Each branch carries its own day-length, heat-sum,
