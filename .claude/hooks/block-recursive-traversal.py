...
# ordinary prose: on 2026-08-14 this hook denied a command whose only trigger was the English word
# "tree" inside `echo "=== main's working tree on those paths ==="`. A shell command name appears
# at the start of the string or a line, after a separator (`|`, `&`, `;`, `(`, `&&`, `||`), or
# behind a wrapper (`sudo`, `time`, `nohup`, `xargs`) -- nowhere else. Third prose-as-code false
# positive in this hook; the first two were a commit message and an inline-python argument.
...
def main():
    ...
    def _flagged(cmd_re, flag_re, long_re=None):
        ...
        Scoped to the span from the command word to the next separator, because searching the whole
        line associates a flag with a command it does not belong to. On 2026-08-14 this denied
        `python plot.py --tape <path> | grep -E "..." ; rm -rf ./tmp/x` -- the `-r` came from `rm
        -rf` and made the unrelated `grep -E` look recursive. Fourth false positive in this hook
        from matching tokens without binding them to their command, after the commit message, the
        inline-python argument, and the word "tree" in an echo string.
        """
