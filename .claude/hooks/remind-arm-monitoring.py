...
MESSAGE = (
    ...
    "missed once:\n"
    "  1. THREE LAYERS: (a) nohup the watcher script, which must PUBLISH A STATE FILE; (b) arm "
    "`Monitor` on its LOG; (c) check the state file's mtime HEARTBEAT with "
    "`tools/check_watcher_state.py` (STALE and DIED both exit non-zero). Layer (c) is the one "
    "that gets dropped and the one that matters: layers (a) and (b) cannot see the WATCHER "
    "die, because a log that stops growing is byte-identical to a job running quietly. A "
    "258-task array once sat finished and unnoticed for ~19 h for exactly that reason. "
    "Liveness is the heartbeat, not a shell trap -- SIGKILL cannot be trapped. A poll loop "
