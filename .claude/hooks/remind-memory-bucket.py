...
def main():
    ...
    if "**Source:**" not in text and not is_user_memory and _added_since(real, SOURCE_SINCE):
        problems.append(f"no `**Source:**` line naming the log this came from (by PATH, not bare stem). "
                        f"Required since {SOURCE_SINCE}; that log must also list this memory under "
                        f"`**Memory written:**`, and both halves are checked.")
