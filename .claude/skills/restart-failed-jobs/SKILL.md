...
modes:
  requires_fates: false      # ELM with OR without FATES — but ELM-family and CIME-submitted
  nutrient_pathway: any
  scope: [hpc]
  summary: "Restart workflow for an ELM-family (CIME-submitted) A2MC ensemble. The REASONING transfers to any model; the SCRIPTS do not — they hardcode ELM's restart glob and CIME's submission path. See §Scope."
---
...
## Scope — ELM and its variants (read this before Step 0)

**This skill's PROCEDURE is written for ELM and ELM-FATES, submitted through CIME.** The
decision logic is general; the tooling is not, and the difference matters because the tooling
fails *silently* off-model rather than erroring.

| Layer | Transfers to another model? |
|---|---|
| **The reasoning** — diagnose before restarting; separate INFRASTRUCTURE failures (`NODE_FAIL`, `PartitionDown`, SIGKILL clusters — restart-eligible) from MODEL failures (an abort that will recur — not restart-eligible without a parameter or source fix); two-wave zombie cleanup; audit before submit | **Yes, fully.** This is the part worth reading for any model. |
| **The failure fingerprints** (Step 2) | Partly. FATES/PARTEH abort strings are examples; another model aborts with its own signatures. |
| **The SCRIPTS** | **No.** `tools/diagnose_ensemble_status.py` hardcodes ELM's restart glob `*.elm.r.*.nc` (`:159`), and restart submission goes through `submit_phase0.py --cases-file`, a CIME path. |

**For an ELM-LIKE model (CLM, CTSM, another CIME land model):** the rules here apply almost
verbatim — same scheduler semantics, same restart-file idea, same infra-vs-model split. Expect to
**adapt or rewrite the scripts** for that model's own restart filename pattern and submission
command; do not assume the ELM ones run.

**For a non-CIME adapter model (EcoSIM, PFLOTRAN, ATS):** the scripts do **not** apply at all, and
they will not tell you so. EcoSIM writes `*.ecosim.r.*.nc` and submits via a SLURM **array**
(`case_template/submit_ensemble_array.sh`), so the ELM glob matches nothing and every case reports
as never-run — a clean-looking "total failure" verdict produced by a pattern that cannot fire.
**Follow the reasoning column above by hand; do not run the commands below.** A parallel
array-aware tool is queued (`TODO.md` → Next, "Parallel restart/status tool for ARRAY-submitted
(non-CIME) ensembles"); until it exists, this skill has no non-CIME path.
...
```bash
source use_cases/{Model}_{Case}/config/<site>_config.sh   # your site (and round) config;
                                                          # it auto-sources its machine config (v2.306)
...
> it diffs what the script prepped vs what actually reached the queue, and emits a **resubmit-only** script
> for the un-submitted `(case, phase)` set (add `--verify-prep` to confirm each case's prep is intact
> first). Filesystem-state-based like `diagnose_ensemble_status.py` but keyed on *submission reach*, not
...
- **`phase0-design`** — owns ensemble submission and launch-mode selection. A restart is a
  submission too: the mode, throttle and submit-ceiling arithmetic recorded in its Step 3
  apply unchanged when resubmitting a failed cohort.
