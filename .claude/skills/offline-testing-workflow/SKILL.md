...
Per the originating dev/ana log §"HPC submission details", set up FOUR dedicated dirs to keep the experiment cleanly separated from the parent Morris ensemble:

| Purpose | Path pattern |
|---|---|
| Param files | `$A2MC_OUTPUT_ROOT/ParameterFiles/fates_params_<exp_name>_<YYYYMMDD>/` |
| Case scripts **+ run/submitter logs** | `$A2MC_SCRIPTS_DIR/<ExpName>_<YYYYMMDD>/` — the launcher/submitter running log (the `<submitter_log>` armed in Step 9c) lives HERE, with the scripts. **Not** in `~` and **not** a system tmp. |
...
export A2MC_CASE_SCRIPTS="$A2MC_SCRIPTS_DIR/<ExpName>_<YYYYMMDD>"
...
Also write a flat manifest TSV (`<repo>/tmp/<exp_name>_manifest_<YYYYMMDD>.tsv` — the **repo-relative** `tmp/`, e.g. `~/A2MC-main/tmp/`, NOT a system/random `/tmp`) so downstream analysis can map variant_id → overrides → nc_file deterministically. (Every `tmp/` path in this skill and in `arm-hpc-monitoring` means the repo's `tmp/`, e.g. `~/A2MC-main/tmp/`. The job-scripts-folder rule for the running log is scoped to **these small offline experiments** — an **ensemble-scale** run keeps its auto-monitor/submitter logs in `tmp/` per `arm-hpc-monitoring`; don't over-apply the job-scripts convention to ensembles.)
...
Arm a Monitor on the submitter log — `<submitter_log>` is the launcher/submitter running log, which lives **in the job-scripts folder** (`$A2MC_SCRIPTS_DIR/<ExpName>_<YYYYMMDD>/…​.log`, per Step 5), not in `~` — with the standard filter from the `arm-hpc-monitoring` skill:
...
- Comparison plot (use `use_cases/{Model}_{Case}/analysis/<plot_script>.py`)
...
*(Repointed 2026-07-31; the Paths rule below adopted from `main` 2026-08-05 — its principle is generic,
its citations were re-authored because two would be wrong here. See the note at the end of this block.)*

- **Paths: source the configs and read them off the environment — do not look them up in a memory.**
  `source` the machine config, then the ACTIVE round's site config, and every path you need is exported:
  ...
  **A memory can go stale against a config change, so it must never be the authority for a path.**
  The **case-dir vs run-dir** rule is stated inline at the pre-flight step above. For the *machine-level*
  facts the config assumes: clone + sync topology [[reference_local_repos_and_sync]], and never write
  scratch to `~/` on Perlmutter — use CFS ([[feedback_no_temp_files_on_home_use_cfs]]).
- **Case naming:** the `--case-suffix` convention in this skill's own experiment-design section.
- **Config sourcing — source the ACTIVE round's site config, which is the live question on this branch.**
  [[feedback_source_config_order_and_round_selection]] (which round's config is active is named by
  `calibration_rounds.yaml`; its machine-config-first instruction predates v2.306, after which the site
  config auto-sources the machine one) **and [[feedback_two_machine_configs_cime_vs_noncime]]**, which
  still governs which family a NEW site-config template must guard on —
  
  | model | SOURCE THIS | run style |
  |---|---|---|
  | FATES | `a2mc_config.sh` | CIME |
  | **EcoSIM** | **`a2mc_noncime_config.sh`** | standalone binary + namelist |
  | **PFLOTRAN** | **`a2mc_noncime_config.sh`** | standalone binary + deck + thermodynamic database |

  `main` dropped this second citation on 2026-08-05, correctly for a FATES-only branch. **It is retained
  here because adapter-kit serves EcoSIM and PFLOTRAN.** The failure it prevents: `a2mc_config.sh`
  *defaults* `A2MC_MODEL_PATH` to the E3SM checkout, so sourcing it for a **non**-CIME model silently
  points the model path at E3SM — no error, a wrong path that surfaces much later.

> **Adoption note (2026-08-05).** `main` rewrote this block (`751bf681`) and the *principle* — read paths
> off the sourced environment, never from a memory — is generic and was taken. The *text* was not: it
> cites `reference_main_tmp_cfs_scratch_path`, which **does not exist on this branch** (ours is
> `feedback_no_temp_files_on_home_use_cfs`), and it drops the CIME/non-CIME citation that adapter-kit
> depends on. Copying it verbatim would have created a dead memory link and lost a live rule.
...
- 2026-08-26: **The two-step source order is now optional, and this file says so.** v2.306 gave every shipped site config a guard that auto-sources its own machine config when one is not already loaded, and REPAIRS the wrong one if it was sourced by mistake. Nothing here was wrong -- the explicit machine-then-site order still works and still takes precedence -- so the instruction is shortened and the old form kept as a stated no-op. Asserted by `tests/test_site_config_autosource.py`. PI-directed. The config-sourcing cross-reference is reframed: the live question on this branch is WHICH ROUND's site config is active (named by `calibration_rounds.yaml`), not which machine config to type. [[feedback_two_machine_configs_cime_vs_noncime]] still governs, but now for authoring a NEW site-config template's guard rather than for a daily source command.

- 2026-08-05: **Adopted `main`'s Paths rule, re-authored rather than copied** (`main` `751bf681`). The
  principle is generic and correct: **source the machine + active-round site configs and read every path off
  the environment; a memory can go stale against a config change, so it is never the authority for a path.**
  The text was NOT taken verbatim — it cites `reference_main_tmp_cfs_scratch_path`, which does not exist on
  this branch (ours is `feedback_no_temp_files_on_home_use_cfs`), and it drops
  `feedback_two_machine_configs_cime_vs_noncime`. Dropping that is right on a FATES-only branch and wrong
  here, where EcoSIM and PFLOTRAN are non-CIME and the wrong machine config silently shadows
  `A2MC_MODEL_PATH`. Both differences are recorded inline as an adoption note.
