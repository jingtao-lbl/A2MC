...
> **Paths are relative to the repo root.** The KB files you edit (`use_cases/{Model}_{Case}/memory/gained_knowledge/`,
...
|---|---|---|---|
| **Site discovery** | `use_cases/{Model}_{Case}/memory/gained_knowledge/discoveries.json` | a site-specific finding / mechanism observed in runs | `MemoryManager.get_relevant_context()` matches its `affects` against `failing_targets` (Phase 3/4/6) |
...
- **`round-housekeeping`** — the POST-ROUND curation step (after the Phase-6 human gate, before the
  next Phase 0) that SEQUENCES this skill as one of its steps. It does not restate the procedure:
  this skill remains its single definition. `round-housekeeping` additionally ASSERTS the outcome —
