...
- 2026-09-08 (later): **Step 1 gains `tools/check_graph_coverage.py`; Steps 0 and 3 gain two corrections the first full audit produced.** Running Step 1 across all four registered profiles found both adapters clean and both FATES profiles carrying the same two unread fields, and then found something the skill had asserted wrongly: FATES's DECLARED seed is not its EFFECTIVE one, because `build_rag_index.py` never passes `curated_yaml_path`. Step 0's table said the per-profile YAML; it now says what is read, with the defect and the `--seed` workaround. Step 3 gains the per-builder rebuild flags after `--graph-only` alone turned out to be a silent no-op for FATES, plus the unrecorded-invocation hold and the reminder to diff the metadata as well as the graph. `20260908j`.

- 2026-09-08: Initial version — distilled from `dev_logs_adapterkit/20260908h_The_Output_Block_Nobody_Read.md`, in which two seed relation blocks were found unread by the EcoSIM builder in one audit (95 edges between them, including the driver of a live round's binding target), after two prior diagnoses had recorded the symptom without finding the cause. Nothing owned the seed-to-graph wiring: `rebuild-rag` owns reindexing and the loud endpoint-not-found skip, `validate-rag-chain` owns content, and neither asks whether a builder reads its whole seed.
