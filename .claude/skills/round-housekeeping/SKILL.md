...
- 2026-09-09: **C-1 gains the input bundle, and the copy-do-not-point rule behind it.** PI-directed. The item listed the parameter values, the base file, the case, the archived binary and the reproduce command — every one of them an identifier rather than the thing identified — so a campaign could satisfy C-1 completely and still be unreproducible once any referenced path moved. Measured 2026-09-09: a converged case's forcing data sat under a `.gitignore`d directory and existed on one machine only. The specification stays in `phase6-refinement` element 7 rather than being restated here, per this skill's SEQUENCE-and-VERIFY scope. No `description` change.

- 2026-09-08: **Step 6 asserted ONE store and printed FOUR (PI).** Its failure condition named `failed_approaches` only, so a round could pass housekeeping holding nine recorded refutations and `experiments: []` — measured, that is the exact state one case was in after twenty experiment cycles. Step 6 now states the failure condition per store, in a table, and adds the **run ledger** (`use_cases/{Model}_{Case}/memory/run_records.json`, `tools/record_run_status.py`) as a separate check, because it is a different kind of object: ungated, written by Phase 5 as the runs progress, and never curated here. Housekeeping's job on the ledger is to notice it is SHORT, since a gap there means a Phase 5 did not record and the round has lost what it ran.
