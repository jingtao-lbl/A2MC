...
**This enum is a CONTRACT, and `tests/test_wrap_scaffold_board_contract.py` now binds the scaffold to it** by parsing the sentence above rather than restating the five names, so neither side can drift alone. It had drifted: the generated `state.py` accepted four values using `open` where this says `pending`, with no `blocked` at all — so a board could not express a blocked task, and `next` answered *what is open* rather than *what is ready*, which pushes the real dependency into prose inside a note where nothing can read it. `blockedBy[]` exists precisely so that question is machine-answerable.

**A scaffold fix does not reach an EXISTING project.** `--refresh` replaces the framework half and never the project's own `scripts/`, which is correct because the project owns them — so migrating a live board is a deliberate step: check the scripts are still byte-identical to as-generated before regenerating them, and merge rather than copy if they are not.
