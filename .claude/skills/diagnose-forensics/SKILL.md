...
- 2026-09-05: **Trigger tightened and the body de-FATES'd** (PI-directed, after a review of whether this skill should exist at all). The evidence said keep it: it has been invoked twice with recorded outcomes, one of which routed into model-evolution work, and `phase3-diagnosis` uses it as the contrast that defines its own scope, so deleting it would have made the surface less clear, not more. What was actually wrong was overlap and FATES assumptions. **Trigger:** dropped "edge-case / collapse detection" and "root cause of this PFT not calibrating" — both whole-round, both `phase3-diagnosis`'s job — and the description now says so explicitly. **Body:** "Morris range" generalised to the round's sampled design (this branch runs `sobol_seq`); the `_exp` suffix stated as per-campaign rather than a literal; `lnd.log` generalised to the model's own run log; the FATES-only screening artifacts (`Set_ID`, `best_case_num`, `_results.txt`, `Sim_` columns) replaced by the rule behind them; `docs/fates-knowledge-base/` replaced by the five KB surfaces plus the source-confirmation step (v2.376); the tool table split into a CIME column and an adapter column, with the mechanism row marked as having no adapter analog rather than pretending otherwise. **Write-up routing corrected:** a calibration finding goes to the case's phase log, which is where the round's own reader looks. Two triage checks added from this round's own failures: truncated or mis-windowed output, and the scheduler-CLI parsing trap.
