...
the checked-out model SOURCE (`file:line`), not just the parameter's long_name/units (PI, 2026-07-19).**

> **KB FIRST, SOURCE TO CONFIRM — a SEARCH ORDER, not a demotion of source (PI, 2026-09-05).** This
> paragraph used to read "not just RAG", which is true about AUTHORITY and was misread as an order of
> operations: source outranks the KB on what the model DOES, so go to the Fortran last, not first.
> **Start at the model's knowledge base — all FIVE of its surfaces, enumerated below** — because it
> usually carries the `file:line` you were about to go find, and it carries CONTEXT the source does
> not: which axis a dimension belongs to, which of two similarly-named axes applies, what a prior
> round already measured. Then read the source to CONFIRM, and cite it. The KB is where the last
> person who read this source wrote down what they learned; re-deriving it is paying twice.
...
The Fortran is the ground truth, and it has repeatedly
