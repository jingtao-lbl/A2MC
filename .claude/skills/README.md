...
|---|---|---|
| [create-project-agent](create-project-agent/SKILL.md) | any | Stand up a PROJECT AGENT: an `A2MC-<Name>` repo whose framework half arrives by sync and whose one top-level project folder holds everything the project authors. Use for a project where calibration is one step among others, or where there is none at all. |
...
| [a2mc-init](a2mc-init/SKILL.md) | any | First run in a CLONE — greet, wire the clone (commit hooks, your name), get + build + verify the model on this machine, offer fork-safe remotes, write the machine config, then route to `onboard-model` / `onboard-case` ("set up A2MC", "get started", "wire up my clone"). |
...
| [arm-local-monitoring](arm-local-monitoring/SKILL.md) | any | Watch an ensemble running on a WORKSTATION with no scheduler — the local counterpart of `arm-hpc-monitoring`; Monitor on the dispatcher log, liveness from the process table, and which of the HPC watcher contract does NOT transfer |
