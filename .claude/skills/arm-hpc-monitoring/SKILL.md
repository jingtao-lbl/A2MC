...
- 2026-08-27: **The `× 3` in the headroom formula is explained as FATES's chain length, not a
  scheduler constant** (PI-directed). The table had carried the multiplier bare since the skill was
  written, when FATES was the only model, and an adapter model is one job per case — so inheriting
  it meters an 8704-case PFLOTRAN round as 26,112 jobs and crawls, while inheriting `× 1` on FATES
  overshoots the ceiling and aborts partway. Adds the per-family table, the array caveat (each task
  counts; an array buys concurrency control, not ceiling headroom), the array-expanded `squeue -r`
  requirement, and the reserve.

- 2026-08-21 — The standalone-binary note now flags **`--ensemble-jobs` as required for arrays and
  task-farms**: the tool's scheduler-reconciliation leg reads per-case `job_id.txt`, which neither
  layout writes, so without it a dead case reads as `RUNNING` indefinitely.

- 2026-08-17 — **Anti-pattern #5 extended: a quiet log is not a running job**, plus a job-ARRAY launch recipe in the standalone-binary note. A 258-task array finished and sat unnoticed ~19 h because the armed tail belonged to an ENDED session, the watcher died without writing its terminal line, and the milestone filter matched EXACT counts while tasks finish in bursts (7 of 8 missed). Names `tools/watch_slurm_array.sh` (state file + heartbeat + `-x` refresh hook) and `tools/check_watcher_state.py`; states that liveness is the HEARTBEAT, not a shell trap (SIGKILL is untrappable), and that a log-only watcher is a weaker signal than a periodically refreshed extract-and-plot pass. Also applied the new `refine-skill` "case references need context" guardrail to this file: the bare `R4/R5` round labels and a site-specific `regen_milestone_plot.sh` path are now generic. Signal: PI question about why the pattern had changed, which surfaced that it had changed silently. Details: `memory/dev_logs_adapterkit/20260815f_*`.
...
- 2026-07-12 — Added a note on standalone-binary model ensembles (EcoSIM): poll `tools/model_ensemble_status.py` (Slurm state reconciled with the model tape check) instead of the FATES auto-monitor/submitter logs.
