...
# Write beside this script, i.e. into the repo's own plot/ directory. Previously both
# paths below were hardcoded to one machine's home tree, which broke the script for every
# other user AND shipped that directory layout to the public repo -- the sync's path
# genericizer rewrote the prefix to '~', which savefig does not expand, so the "fixed"
# form was a literal directory named '~'. Derive it instead.
...
# Optional second copy, e.g. into a companion project. Opt-in via env var rather than a
# hardcoded sibling path, which only existed on the original author's machine.
_extra = os.environ.get('A2MC_PLOT_EXTRA_DIR')
if _extra:
    output_file2 = str(Path(_extra).expanduser() / 'A2MC_Workflow_Horizontal.png')
