...
def parse_targets_yaml(yaml_file) -> Dict[str, Target]:
    ...
    for name, spec in config.get('targets', {}).items():
        ...
        targets[name] = Target(
            ...
            # Backend-dispatched extraction specs (None for FATES SZPF targets, which
            # derive the variable/PFT from the target name). Adapter models set these.
            variable=spec.get('variable'),
            pft=spec.get('pft'),
            window=spec.get('window'),
            reduce=spec.get('reduce'),
            time_index=spec.get('time'),
            denominator=spec.get('denominator'),
            observed_series_file=spec.get('observed_series_file'),
        )
