...
CHECKS = [
    ...
    # array-watcher pair documented by `arm-hpc-monitoring` (2026-08-17). Both are read-only here:
    # --help never touches a scheduler, and `bash -n` only parses. They are covered because the
    # skill now tells a reader to LAUNCH them — an undocumented-but-broken backing command is what
    # Tier-2 is for.
    ("check_watcher_state --help", "monitoring", "sys", ["tools/check_watcher_state.py", "--help"], "usage"),
    ("watch_slurm_array syntax", "monitoring", "bash", ["-n", "tools/watch_slurm_array.sh"], None),
    ...
    # The two commands the PFLOTRAN phase-skill edits of 2026-08-27 newly DOCUMENT. A skill that
    # names a command the harness never runs is a path-check away from a broken instruction:
    # Tier-1 proves the file exists, only this proves it still starts.
    ("validate_pflotran_ensemble --help", "calibration", "a2mc",
     ["scripts/validate_pflotran_ensemble.py", "--help"], "usage"),
    ("submit_adapter_ensemble_batched --help", "calibration", "a2mc",
     ["scripts/submit_adapter_ensemble_batched.py", "--help"], "usage"),
...
def interp_path(kind):
    ...
    if kind == "bash":
        # Some documented backing commands are shell scripts, not Python. Without this the Tier-2
        # gate silently cannot cover them: it would report "clean" for a skill whose newly
        # documented command is a broken shell script, which is the same class of non-failing
        # check this harness exists to prevent.
        import shutil
