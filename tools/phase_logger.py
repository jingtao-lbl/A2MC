...
class PhaseLogger:
    ...
    def __init__(self, site_dir: str = None, site_name: str = None,
        ...
        # Resolve site name.
        #
        # The case DIRECTORY NAME is the third source, and it is what makes this work for an
        # adapter model. Only the FATES site configs export A2MC_SITE_NAME
        # (use_cases/ELM-FATES_template/config/fates_template_config.sh); every EcoSIM and PFLOTRAN
        # config exports A2MC_SITE_CONFIG and not A2MC_SITE_NAME, so the old two-source lookup
        # fell through to the literal string 'Unknown' for every adapter case. Measured
        # 2026-08-20: 88 of 126 committed EcoSIM_BioCON logs are headed `**Site:** Unknown`.
        #
        # Deriving from self.site_dir needs no per-case config edit and fixes every existing
        # adapter case at once. The env var still wins where it is set, so the FATES path is
        # byte-identical (Kougarok keeps 'Kougarok' rather than becoming its directory name).
        self.site_name = (
            site_name
            or os.environ.get('A2MC_SITE_NAME')
            or (self.site_dir.name if self.site_dir and self.site_dir.name else None)
            or 'Unknown'
        )
        ...
        # MODE MISMATCH GUARD. `A2MC_AGENT_MODE` defaults to 'online' and NOTHING in any config
        # sets it -- measured 2026-08-22: its only setters in the whole repo are tests. So the
        # offline agent must export it by hand every session, and forgetting is silent: the log
        # goes to `logs/{session_id}/phase{N}_{name}/...` while `topic_artifact_dir()` keeps
        # returning the offline stem, producing a `phase_results/{stem}/` with no matching
        # `logs/{stem}.md` -- exactly the orphan the stem invariant forbids.
        #
        # Decidable, so it need not be left to discipline: if this logger is about to write in
        # ONLINE layout into a log_dir that already holds OFFLINE-stem logs, the mode is wrong.
        # Warn rather than raise -- a site may legitimately be run by both agents over its life,
        # and erroring would block the autonomous loop on a bookkeeping artifact.
        self._mode_guard_pending = (not self._offline and agent_mode is None)
        ...
        if getattr(self, '_mode_guard_pending', False):
            self._warn_if_offline_layout_present()
    ...
    def _clean_descriptor(descriptor: str) -> str:
        ...
        s = '_'.join(s.split())
        # STRIP A TRAILING SEPARATOR LEFT BY THE TRUNCATION. The 60-char cut lands wherever it
        # lands, and when it lands on the '_' between two words the descriptor ends in '_'. That
        # produces a stem no hand-typed path ever matches, and the pairing between logs/{stem}.md
        # and phase_results/{stem}/ is the whole point of the stem -- so the two silently diverge
        # and the state's log_path stops resolving. Measured 2026-08-23: two of one cycle's three
        # titles hit it, costing three renames. Non-retroactive by nature; stems already on disk
        # are found by _existing_offline_letter's suffix match, which is unaffected.
        return s[:60].rstrip('_') or 'topic'

    #: An offline log stem: YYYYMMDD + letter(s) + _phase<N>_. Matching this in `logs/` means a
    #: previous session wrote here in offline mode.
    _OFFLINE_STEM = re.compile(r"^\d{8}[a-z]+_phase\d_")

    def _warn_if_offline_layout_present(self) -> bool:
        """True (and warns) when writing online into a directory that holds offline-stem logs."""
        try:
            if not self.log_dir.is_dir():
                return False
            hits = [p.name for p in self.log_dir.glob("*.md")
                    if self._OFFLINE_STEM.match(p.name)]
        except OSError:
            return False
        if not hits:
            return False
        warnings.warn(
            f"PhaseLogger is in ONLINE mode but {self.log_dir} already holds "
            f"{len(hits)} OFFLINE-stem log(s) (e.g. {hits[0]}). If you are the interactive agent, "
            f"A2MC_AGENT_MODE is unset -- export A2MC_AGENT_MODE=offline. Writing now would put the "
            f"log under logs/<session>/phase<N>_<name>/ while topic_artifact_dir() keeps the offline "
            f"stem, leaving a phase_results/<stem>/ with no matching logs/<stem>.md.",
    ...
    def topic_stem(self, phase: int, descriptor: str) -> str:
        ...
        # REUSE an existing stem for the same (date, phase, round, loop, descriptor) before
        # allocating a new letter. The in-memory cache above is PER-PROCESS, so without this a
        # second process -- the usual shape: one run creates phase_results/, a later one writes
        # the log -- allocates the NEXT letter and the log silently stops matching its own
        # artifact folder. That happened on 2026-08-22 (phase 1) and cost a rename of the folder
        # plus 18 repointed state references. The pairing is supposed to be stable; make it so.
    ...
    def _existing_offline_letter(self, today: str, suffix: str):
        ...
        Matches on the FULL suffix (phase, round, loop counters, descriptor), so two different
        topics on the same day never collide onto one letter.
        """
...
"""
    ...
    #
    # EVERY phase gets a list, not only the analysis phases 3/4/6 (PI, 2026-08-02). A
    # phase-0 log is not a one-line "submitted" note: it is the durable record of what was
    # submitted, which job/array IDs, what monitoring was armed, which cases failed when the
    # scheduler hiccupped, what was restarted and how, and the early plots that checked the
    # run looks right — all paired with artifacts in `phase_results/{stem}/`. Without those
    # sections named, a phase-0 log that omits them looked complete.
    # Both phases that PUT SIMULATIONS ON A SCHEDULER (0 the ensemble, 5 the experiments)
    # share this run-and-watch spine. Each item is unrecoverable if deferred to the end of the
    # phase — nobody reconstructs an array ID, a failed-case list or a restart command from
    # memory a week later.
    #
    # NOT in the spine: "Cases Materialized" is phase-0 vocabulary (PI, 2026-08-02). Phase 0
    # materializes an ensemble of per-case parameter files; phase 5 designs a handful of
    # VARIANTS from the hypothesis the inner 3↔4 loop produced, so what it must show is the
    # variants' design, their run status, and a preview confirming the new tests came out sane.
    _RUN_SPINE = ["Submission", "Simulation Status", "Monitoring Armed",
                  "Failures and Restarts"]

    # "Parameter Set and Bounds" leads phase 0 because it PRECEDES sampling: Step 1 samples
    # whatever the CSV says, so add/drop/bounds is the decision that determines everything
    # after it. Its sharpest content is what a DROPPED parameter is now fixed at -- dropping
    # one does not fall back to the CSV `default` (documentation); the backend copies
    # $A2MC_BASE_PARAM_FILE and applies only the matrix edits, so control of that value moves
    # to the base file. That has bitten twice (20260716a: 4 non-calibrated EcoSIM params flowed
    # from the base as-is, one a 10x discrepancy no checker flagged; and the api-31->api-43
    # port, feedback_port_tuned_base_param_file_across_versions). Required for EVERY phase 0,
    # not only a redesign -- 20260716a was a round-1 failure, and round 1 must equally state
    # what is deliberately NOT calibrated (PI, 2026-08-07).
    #
    # Two of log_design's own sections are deliberately NOT listed. "Parameter Summary" would
    # overlap "Parameter Set and Bounds", and a section that does not earn its place gets
    # answered with filler (the C4 lesson, 20260802a). "Design Decisions" is emitted
    # UNCONDITIONALLY, with a "*No specific decisions recorded*" placeholder when empty -- so it
    # already leaves a visible trace, and listing it would add an entry the gap check can never
    # name (verified: it never appears in the gap list, because `## Design Decisions` is always
    # in the content). An entry that cannot fire implies coverage it does not provide.
    _EXPECTED_SECTIONS = {
        0: ["Parameter Set and Bounds", "Sampling Design",
            "Cases Materialized"] + _RUN_SPINE + ["Verification Plots"],
        ...
        # HEADING NAMES ARE A CONTRACT between the writer (`log_<phase>`) and this list, and they
        # DISAGREED for phase 6: `log_refinement` emits "## AI Reasoning and Deep Analysis"
        # (line ~1886) while this list expected "AI Reasoning and Analysis", so EVERY phase-6 log
        # ever written reported that section as "not provided" even when it was full. The comment
        # that used to sit here asserted they matched, which is how it survived. Measured and fixed
        # 2026-08-22 on the R3 c01 refinement log. A "sections not provided" list that always names
        # a section nobody omitted is noise, and noise trains readers to skip the whole list.
        # NB phase 4 says "AI Reasoning and Analysis"; phases 3 and 6 say "...and Deep Analysis" —
        # matching what each phase method ACTUALLY emits (`log_hypothesis` :1401 and
        # `log_refinement` write the short form; `log_diagnosis` :1140 writes the long one). These
        # entries previously all said "Deep", so every offline phase-4 and phase-6 log listed its
        # own AI-reasoning section as "not provided" while the section was right there under a
        # slightly different name — a false entry in the very list that exists to distinguish
        # "not done" from "not applicable" (found 2026-08-14 on R3 c00, the first offline phase log
        # written since this gap check landed). The remaining naming inconsistency BETWEEN phases is
        # real but cosmetic and unifying it would change emitted log format, so it is left alone.
        4: ["Mechanism", "Parameters to Modify", "AI Reasoning and Analysis",
    ...
    def _offline_supplement(self, phase: int, content: str) -> str:
        ...
        distinguishable from "not applicable" — the same reason spec_na exists.
        """
    ...
    def log_design(self,
                   ...
                   parameter_set_changes: str = None,
        ...
        Args:
            ...
            parameter_set_changes: The parameter-set design decision, as Markdown -- what was
                ADDED and on what evidence, what was DROPPED **and the fixed value it now takes
                and where that value comes from**, each bound's `bound_source`, and whether each
                range is physically plausible at this site. Emits `## Parameter Set and Bounds`.
                Optional and source-compatible, so callers that omit it are unaffected -- they
                simply get the section named in the gap list.
...
"""
        ...
        # Emitted ONLY when supplied. Always emitting the heading would satisfy the
        # `f"## {s}" not in content` gap check with an empty placeholder, i.e. the section
        # would report itself as provided while carrying nothing -- a check that cannot fail
        # (feedback_a_check_that_cannot_fail). Same conditional shape as parameter_summary.
        if parameter_set_changes:
            content += f"""
---

## Parameter Set and Bounds
...
"""
            # `selected_base_cases` IS THE CURRENT CONTRACT and this renderer was one version
            # behind it from v2.80 (2026-04-07) until 2026-08-22. `reasoning/methods.py` emits it
            # and `orchestrator.py:1860` reads it as primary, treating the three scalar keys below
            # as explicit backward compatibility -- but this block read ONLY the scalars, so a
            # diagnosis supplying the current schema lost its comparative analysis and rendered
            # three "#N/A" bullets instead. That is worse than an empty section: it reads as "no
            # comparison was made" when two fully specified base cases were handed over. Caught by
            # the PI reading a rendered log, not by any check -- see check_log_placeholders.py,
            # added the same day so it cannot recur silently.
            selected = comparative_analysis.get('selected_base_cases') or []
            for sel in selected:
                cid = sel.get('case_id', '?')
                content += f"- **Case #{cid}**"
                sat = sel.get('targets_satisfied') or []
                fix = sel.get('targets_to_fix') or []
                if sat or fix:
                    content += f" — satisfies {', '.join(map(str, sat)) or 'none'};"
                    content += f" to fix: {', '.join(map(str, fix)) or 'none'}"
                content += "\n"
                if sel.get('rationale'):
                    content += f"  - {sel['rationale']}\n"
            if selected:
                content += "\n"

            # The legacy scalars, emitted ONLY when actually supplied. An absent value is now
            # omitted rather than printed as "#N/A"; a placeholder that looks like a finding is
            # the failure this whole block is being rewritten for.
            for label, key in (("Best case (targets)", 'best_case_id'),
                               ("Lowest cost case", 'lowest_cost_case_id'),
                               ("Recommended starting case", 'recommended_starting_case')):
                val = comparative_analysis.get(key)
                if val not in (None, '', 'N/A'):
                    content += f"- **{label}:** #{val}\n"
            if comparative_analysis.get('rationale'):
                content += f"- **Rationale:** {comparative_analysis['rationale']}\n"
            if not selected and not any(
                    comparative_analysis.get(k) for k in
                    ('best_case_id', 'lowest_cost_case_id', 'recommended_starting_case',
                     'rationale')):
                content += ("_(comparative_analysis was supplied but carried no recognised key — "
                            "expected `selected_base_cases`, or the legacy `best_case_id` / "
                            "`lowest_cost_case_id` / `recommended_starting_case`.)_\n")
            content += "\n"
...
"""
        # ACCEPT THE SYNONYMS RATHER THAN SWALLOWING THEM. Until 2026-08-22 this read only
        # 'name'/'current'/'proposed' and rendered "### Unknown / Current: N/A / Proposed: N/A"
        # for anything else, which is how 20 phase-4 hypothesis logs across R1, R2 and R3 lost the
        # IDENTITY and the VALUES of every parameter they proposed, keeping only the rationale.
        # The data was in the call and the log threw it away; nothing reported it, because a stub
        # renders as successfully as a value. Synonyms are read in order, and an unrecognised dict
        # is now dumped verbatim (below) instead of being replaced by placeholders --  a log that
        # is ugly and complete beats one that is tidy and empty.
        def _first(d, *keys):
            for k in keys:
                v = d.get(k)
                if v not in (None, ''):
        ...
        for param in parameters_to_modify:
            ...
            if name is None and current is None and proposed is None:
                ...
                # content survives and the key mismatch is visible to whoever reads the log.
                content += "### (unrecognised parameter entry — keys not understood by the logger)\n"
                content += f"```\n{param!r}\n```\n\n"
                continue
            name = name if name is not None else 'Unknown'
            current = current if current is not None else 'N/A'
            proposed = proposed if proposed is not None else 'N/A'
            pft = param.get('pft')
...
"""
        ...
        # `Hypothesis.success_criteria` (reasoning/schemas.py) is the FALSIFICATION threshold —
        # what would have to be true for the hypothesis to be judged confirmed, and what would
        # refute it. It existed in the schema but this logger never accepted or emitted it, in
        # EITHER agent mode, so Phase 6 later ruled CONFIRMED/REFUTED against a bar that was
        # never written down. Expected outcomes say what we think will happen; success criteria
        # say what settles it. (Found 2026-08-02.)
        if success_criteria:
            content += f"""
---
