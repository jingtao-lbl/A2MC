...
class Target:
    ...
    # ---- Backend-dispatched extraction specs (adapter models; docs/38) ----
    # For a model driven through a ModelBackend (EcoSIM, …), the screening extractor
    # reads the history variable + grouping-axis slot + time reduction straight from the
    # target, then calls backend.extract_history_variables + model_evaluate_case.reduce_target.
    # FATES leaves these None and derives the variable/PFT from the target NAME (SZPF).
    variable: Optional[str] = None          # history var on the tape (e.g. "LEAF_C_pft")
    pft: Optional[int] = None               # 1-based grouping-axis slot (None = column-level)
    window: Optional[List[int]] = None      # [lo, hi] inclusive 0-based index range (peak-season)
    reduce: Optional[str] = None            # "last" | "mean" | "max" over the window (default mean)
    time_index: Optional[int] = None        # 0-based snapshot index (alternative to window)
    # Second column a DERIVED reduce forms a ratio against (backend.MODEL_REDUCES).
    # PFLOTRAN's outflow concentrations divide a solute rate by the water rate at the
    # SAME coupler, which cancels the unresolved area normalisation exactly.
    denominator: Optional[str] = None
    # Path (repo-relative or absolute) to a full observed TIME SERIES a derived reduce
    # scores against directly, rather than a single scalar `observed` value. PFLOTRAN's
    # `outflow_flux` is the first user: a two-column [day, mm/h] measured hydrograph.
    observed_series_file: Optional[str] = None
    ...
    with open(results_file, 'w') as f:
        ...
        for rank, idx in enumerate(result.ranked_indices, start=1):
            ...
            for name in target_names:
                ...
                # `%.2f` DESTROYS the value for any model whose outputs are small numbers, and it
                # destroys it SILENTLY. Written for FATES biomass in g C/m^2, where two decimals are
                # ample. PFLOTRAN's outflow concentrations are ~4.7e-04 mol/L, so every Sim_ column
                # printed `0.00` while the RE_ columns beside them were correct -- measured
                # 2026-08-27 on miniLEO, where 10 of 11 targets came out as zeros and only the
                # O(1) hydrograph sentinel survived.
                #
                # This file is the de-facto Y MATRIX: a sensitivity or surrogate step reading the
                # Sim_ columns would have been handed a matrix of zeros with one real column, and
                # zeros look like data. `%.6g` keeps ~6 significant figures at any magnitude, which
                # is what a value destined for numerical analysis needs, and still prints FATES's
                # biomass readably.
                row.append(f"{sim_repr:.6g}")
            f.write("\t".join(row) + "\n")
