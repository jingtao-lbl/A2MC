...
def _default_of(field_name: str):
    """The dataclass default for a Milestone field, for absence-vs-set comparisons."""
    import dataclasses
    for f in dataclasses.fields(Milestone):
...
class Milestone:
    ...
    # --- ROUND-TRIP FIDELITY (added 2026-08-27) ---------------------------------
    # This dataclass is a FATES-shaped typed VIEW of a registry that is no longer
    # FATES-only: `rag/milestones.json` carries adapter profiles (ecosim, pflotran)
    # with keys this class has no field for -- `adapter`, `model_commit_built`,
    # `model_wiki_subdir`, `param_file`, `param_file_format`, `output_cdl`,
    # `knowledge_base_dir`, `upstream_url` -- plus `collection_name` and
    # `expected_counts` on EVERY profile, the latter being what
    # `tools/check_rag_index_committed.py` reads.
    #
    # Before this, load->save was lossy in BOTH directions: it dropped those keys
    # AND invented ~14 empty `fates_*` keys on an adapter entry, turning a PFLOTRAN
    # milestone into a malformed FATES-looking one. Nothing had fired only because
    # both adapter builders bypass this class and read the JSON directly
    # (build_ecosim_rag.py:59, build_pflotran_rag.py:67) -- so the workaround hid
    # the defect rather than avoiding it, while `rag_manifest.py` and `rag_bump.py`
    # kept a live write path through here.
    #
    # `extra` preserves unmodelled keys; `_source_keys` preserves the ORIGINAL key
    # order and, crucially, WHICH KEYS WERE PRESENT -- so a profile that never had
    # `fates_api_epoch` does not acquire one. Both are excluded from the emitted
    # JSON body itself.
    extra: Dict[str, Any] = field(default_factory=dict)
    _source_keys: Optional[List[str]] = None

    # Keys this class models, in the order to_dict emits them for a NEW milestone.
    _MODELLED = ("description", "fates_api_epoch", "fates_tag_built", "fates_commit_built",
                 "fates_param_file_format", "elm_commit_built", "elm_wiki_subdir",
                 "fates_wiki_subdir", "fates_output_cdl", "elm_output_cdl",
                 "covers_sci_tags", "canonical", "legacy", "available_locally",
                 "published_at", "curated_yaml_path", "notes")

    def _modelled_value(self, key: str):
        v = getattr(self, key)
        return list(v) if key == "covers_sci_tags" else v

    def to_dict(self) -> dict:
        """Emit the registry body.

        For a milestone LOADED from the file this is lossless and order-preserving:
        exactly the keys that were present come back, in their original order, with
        modelled fields taking their current (possibly updated) values. For a
        milestone constructed in code, every modelled key is emitted followed by any
        extras.
        """
        if self._source_keys is not None:
            out = {}
            for k in self._source_keys:
                out[k] = (self._modelled_value(k) if k in self._MODELLED
                          else self.extra.get(k))
            # A modelled field that was ABSENT from the source ships only if it was
            # actually SET IN CODE, i.e. it differs from the dataclass default. Testing
            # truthiness instead is wrong and was caught by the round-trip test: an
            # adapter profile acquired `fates_param_file_format` because its default is
            # the non-empty string "cdl", which is exactly the FATES-shaped key an
            # adapter entry must not grow.
            for k in self._MODELLED:
                if k not in out and self._modelled_value(k) != _default_of(k):
                    out[k] = self._modelled_value(k)
            for k, v in self.extra.items():
                out.setdefault(k, v)
            return out
        return {**{k: self._modelled_value(k) for k in self._MODELLED}, **self.extra}
    ...
    def from_dict(cls, profile_name: str, body: dict) -> "Milestone":
        return cls(
            ...
            extra={k: v for k, v in body.items() if k not in cls._MODELLED},
            _source_keys=list(body.keys()),
        )
