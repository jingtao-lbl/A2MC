...
#: EFFECTIVE DATE. 20 phase-4 hypothesis logs from R1/R2/R3 (2026-07-16 to 2026-08-22) carry
#: "### Unknown / Current: N/A / Proposed: N/A" because `log_hypothesis` read only one spelling of
#: the parameter keys. The renderer is fixed, but those logs CANNOT be repaired: the dicts that
#: produced them are gone, and the log is the only record. Flagging them forever would make this
#: check permanently red, which is how a check stops being read -- the same reasoning as
#: check_skill_claims. They are exempt, and the exempt COUNT IS PRINTED so the backlog stays a
#: visible decision rather than a disappeared one. Queued in TODO.md.
PLACEHOLDER_RULE_EFFECTIVE = "20260822"
