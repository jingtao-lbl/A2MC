...
def _promote_one(mm, item, confidence, verified_by=""):
    ...
    if kind == "discovery":
        ...
        # parameters.json gained the propose gate on 2026-08-25; before that it was the one
        # curated store an unattended run could write directly.
        mm.add_parameter_info(
            item.get("key", ""),
            parameter_pft=entry.get("parameter_pft"),
            affects_pfts=entry.get("affects_pfts"),
            **{k: v for k, v in entry.items()
               if k not in ("parameter_pft", "affects_pfts")})
    elif kind == "parameter_knowledge":
        mm.add_parameter_knowledge(
            item.get("key", ""),
            knowledge_type=entry.get("type", "calibration_advice"),
