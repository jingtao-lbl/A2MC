...
def parse_code_reference(ref: str) -> Tuple[Optional[str], Optional[str]]:
    """Parse a code reference into (file, routine).

    Two conventions are accepted, because the kit is not Fortran-only:
      Fortran/FATES  'File.F90::routine'          -> ('File.F90', 'routine')
      path:line      'src/a/b.hh:65 (class Foo)'  -> ('src/a/b.hh', None)

    The second form is what a C++ model (ATS) and any line-cited wiki produce. Without it, every
    such reference fell through to the whole string being treated as a filename and Dim D scored
    near zero on a perfectly good inventory.
    """
    if not ref:
        return None, None
    ref = ref.strip()
    ...
    # `path/to/file.ext:LINE` optionally followed by prose -- take the path, drop the line.
    m = re.match(r"([A-Za-z0-9_./\-]+\.[A-Za-z0-9]+):(\d+)", ref)
    if m:
        return m.group(1), None
    # A bare path with no line number, possibly followed by prose.
    m = re.match(r"([A-Za-z0-9_./\-]+\.[A-Za-z0-9]+)", ref)
    if m:
        return m.group(1), None
    return ref, None
...
def _model_parsers(model: str):
    """Return (parameter_parser, output_parser) instances for `model`.

    Dispatches through the adapter registry so the kit's V2 gate works for ANY onboarded model.
    Previously this module instantiated FATESParameterParser/FATESOutputParser directly, so a
    non-FATES parameter file died with "Unsupported file format" (ATS decks are .xml) and step 8
    of onboard-model was unreachable for every adapter. FATES stays the default, so existing
    invocations are unchanged.
    """
    if model and model.lower() != "fates":
        import importlib
        importlib.import_module(f"models.{model}")
        from models import registry
        spec = registry.get_model(model).spec
        return spec.parameter_parser_class(), spec.output_parser_class()
    return None, None


def validate_parameters(curated: dict, wiki: WikiCorpus,
                        param_file: Path, model: str = "fates") -> DimensionResult:
    """Dimension A: every YAML parameter appears literally in the wiki."""
    result = DimensionResult()
    yaml_params = curated.get("parameters") or {}

    pp, _ = _model_parsers(model)
    if pp is not None:
        file_params = pp.parse(str(param_file))
    else:
        file_params = FATESParameterParser(str(param_file)).parse()
    ...
    for pname in sorted(yaml_params.keys()):
        ...
        if not wiki_files:
            # A parameter's wiki IDENTITY is its name, not its address. FATES names are bare
            # (`fates_leaf_vcmax`) so the two coincide; a path-addressed model (ATS Teuchos XML,
            # any namelist-with-groups) stores `a/b/c/van Genuchten alpha [Pa^-1]`, which never
            # appears literally in prose. Retry on the leaf, units stripped, before declaring the
            # parameter undocumented -- otherwise every path-addressed model scores 0/N here.
            leaf = re.sub(r"\[[^\]]*\]", " ", pname).rsplit("/", 1)[-1].strip()
            if leaf and leaf != pname:
                wiki_files = wiki.files_containing(leaf)
...
def validate_outputs(curated: dict, wiki: WikiCorpus,
                     output_cdl: Path, model: str = "fates") -> DimensionResult:
    ...
    _, op = _model_parsers(model)
    cdl_vars = set((op.parse(str(output_cdl)) if op is not None
                    else FATESOutputParser(str(output_cdl)).parse()).keys())
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--model", default="fates",
                    help="Adapter model name (default: fates). Dispatches the parameter/output "
                         "parsers through models/<name>/ so the gate works for any adapter.")
...
def main() -> int:
    ...
    dim_a = validate_parameters(curated, wiki, args.param_file, args.model)
    ...
    dim_c = validate_outputs(curated, wiki, args.output_cdl, args.model)
