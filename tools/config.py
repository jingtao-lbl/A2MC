...
Before running Python scripts, source the site config (since v2.306 it auto-loads the
machine config, so this one line is the whole chain):
...
class A2MCConfig:
    ...
    def TOTAL_ENSEMBLE(self) -> int:
        ...
        - lhs:       n_samples
        - sobol_seq: n_samples          (the raw scrambled Sobol' SEQUENCE -- N rows)
        - sobol:     n_samples × (2 × n_params + 2)   (the SALTELLI design)

        `sobol` and `sobol_seq` are different DESIGNS, not a flag on one design, so they
        size differently. Returning 0 for an unrecognised scheme is worse than raising:
        a caller sizing headroom from it silently gets nothing, which is exactly what
        happened for `sobol_seq` on the live PFLOTRAN R1 config until 2026-08-27.
        """
