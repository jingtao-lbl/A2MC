...
def resolve_declared_model(yaml_path) -> str:
    """Which model this targets.yaml belongs to: 'fates' or an onboarded adapter model.

    Order, most authoritative first:
      1. a top-level `model:` key IN THE FILE -- it declares itself;
      2. `$A2MC_MODEL`, but only when explicitly set to something other than fates;
      3. 'fates', the historical default.

    Both sources are needed. The `model:` key is NOT universal -- EcoSIM_Lusignan and
    PFLOTRAN_miniLEO carry it, EcoSIM_BioCON does not (measured 2026-08-18) -- so a
    content-only check would silently mis-handle BioCON, and an env-only check would
    mis-handle a file passed by path with no environment sourced.
    """
    try:
        import yaml as _yaml
        doc = _yaml.safe_load(Path(yaml_path).read_text()) or {}
        declared = str(doc.get("model", "")).strip().lower()
        if declared:
            return declared
    except Exception:
        pass
    env_model = os.environ.get("A2MC_MODEL", "").strip().lower()
    return env_model or "fates"
...
def main() -> int:
    ...
    # REFUSE an adapter model's targets file rather than mis-checking it.
    #
    # This validator is FATES-shaped: rule R1 requires `PFT<id>_<var>` names and S3 requires a
    # `time_year`/`time_month` anchor. An adapter target satisfies NEITHER by construction -- it is
    # named for the quantity (`GPP`, `outflow_Ca`) and anchored with `window_years`/`window` -- so
    # every target produced exactly 2 spurious errors and the aggregate gate could never pass for
    # EcoSIM, PFLOTRAN or ATS (audit: memory/dev_logs_adapterkit/20260816b). Loud-but-meaningless
    # output is worse than a silent miss: it trains readers to ignore the gate.
    #
    # Same precedent as the two LOG checkers, which were made to refuse each other's stream on
    # 2026-08-14. Refusing ROUTES rather than dead-ends -- it names the right tool.
    declared = resolve_declared_model(yaml_path)
    if declared and declared != "fates":
        print(f"wrong tool: {Path(yaml_path).name} belongs to model '{declared}', and this "
              f"validator is FATES-shaped (PFT<id>_<var> names + time_year/time_month anchors).\n"
              f"  Use the model-generic validator instead:\n"
              f"    python tools/validate_model_targets.py --model {declared} "
              f"--targets {yaml_path}", file=sys.stderr)
        return 2
