...
**What this answers.** A2MC ranks a completed ensemble; it has never had a way to ask *what should we run next?* This package is that. It proposes the parameter sets most likely to put **every** validation target inside its observational band at the same time, confirms them on the physics model, and repeats.
...
| **S1** | `bo_replay.py` | the **GO/NO-GO gate**: replay the propose loop against an already-completed ensemble and measure whether it rediscovers a known optimum faster than random | **built** — 17 tests |
| S2 | `acquisition.py` | feasibility-weighted expected improvement, batched; maximized by multi-start L-BFGS-B with gradients through the surrogate, falling back to a random-candidate scan | not built — **gated on S1** |
| S3 | `bo_loop.py` | propose → run → refit, with the three stopping rules | not built |
